CREATE PROCEDURE `adm2025_listOfUserV2`(pvIds TEXT ,pvGroupIds TEXT ,pnGroupFilterType TINYINT(1) ,pvKeyword VARCHAR(256) -- search ON field: email & account_3rd_emails ,pbAccountTypes TINYINT(1) -- 0: Google 1: Yahoo 2: iCloud 3: Other ,pbSubscriptionTypes TINYINT(1) -- 0: Standard 1: Premium 2: Pro ,pdLastUsedStart DOUBLE(13, 3) ,pdLastUsedEnd DOUBLE(13, 3) ,pdJoinDateStart DOUBLE(13, 3) ,pdJoinDateEnd DOUBLE(13, 3) ,pvMigrationStatus VARCHAR(20) -- 0: NOT migrate 1: CREATE new ON DB4.0 2: Migrated ,pvPlatformIds VARCHAR(256) ,pvPlatformFilterType TINYINT(1) -- 0:any, 1:ALL ,pbFloMacUpdate TINYINT(1) ,pvFloMacFilterType TINYINT(1) ,pbIsInternal TINYINT(1) ,pvSort VARCHAR(128) -- [-DESC +ASC] fieldname ,pnOFFSET INT(5) ,pnLIMIT INT(5)) BEGIN
  --
  ...
  SET vGroupIds = CASE
    ...
    END;
  --
  SET nRows = adm2026_listOfUserCount(pvIds
                                     ,bIsNoGroup
                                     ,vGroupIds
                                     ,pnGroupFilterType
                                     ,vKeyword
  ...
  SELECT uu.email -- , vGroupIds, bIsNoGroup
    ...
    LEFT JOIN (
     ...
     WHERE rcu.id IS NOT NULL
       AND (rcu.email LIKE vKeyword OR rcu.account_3rd_emails LIKE vKeyword)
