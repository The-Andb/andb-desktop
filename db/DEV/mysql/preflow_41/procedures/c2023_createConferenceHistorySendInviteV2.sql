CREATE PROCEDURE `c2023_createConferenceHistorySendInviteV2`(pnChannelId BIGINT(20) ,pnUserId BIGINT(20) ,pnType TINYINT(1) ,pvOrganizer VARCHAR(100) ,pvMemberForce TEXT ,pvMeetingId VARCHAR(1000) ,pvExMeetingId VARCHAR(1000) ,pnUpdatedDate DOUBLE(13,3)) BEGIN
  --
  --
  ...
  his_loop: LOOP
    ...
    SELECT ifnull(max(cm.start_time), unix_timestamp(now(3)))
      ...
      INSERT INTO conference_history
          ...
          action_time, conference_meeting_id, meeting_id, external_meeting_id, created_date, updated_date, created_by_server) 
      VALUES
          (nUserID, nCMID, vEmail, nMeetingStartTime, pnType, 24, pvOrganizer
          ,unix_timestamp(now(3)), nMeetingId, ifnull(pvMeetingId, ''), ifnull(pvExMeetingId, ''), pnUpdatedDate, pnUpdatedDate, 1);
      --
