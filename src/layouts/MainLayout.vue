<template>
  <div
    class="h-screen flex flex-col pt-0 bg-white dark:bg-gray-900"
  ...
  >
    ...
    <div class="flex-1 flex overflow-hidden relative">
      ...
      <div
        ...
        :style="{
          width: displaySidebarWidth + 'px',
          borderRightWidth:
            displaySidebarWidth === 0 || appStore.layoutSettings.sidebarPosition === 'right'
              ? '0'
              : '1px',
          borderLeftWidth:
            appStore.layoutSettings.sidebarPosition === 'right' && displaySidebarWidth !== 0
              ? '1px'
              : '0'
        }"
        :class="[
          'shrink-0 relative transition-all duration-300 ease-in-out border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden',
          appStore.layoutSettings.sidebarPosition === 'right' ? 'order-3' : 'order-1'
        ]"
      >
      ...
      <div
        ...
        :style="{
          width: appStore.layoutSettings.aiPanelWidth + 'px',
          borderRightWidth: appStore.layoutSettings.aiPanelPosition === 'left' ? '1px' : '0',
          borderLeftWidth: appStore.layoutSettings.aiPanelPosition === 'right' ? '1px' : '0'
        }"
        :class="[
          'shrink-0 relative transition-all duration-300 ease-in-out border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex flex-col',
          appStore.layoutSettings.aiPanelPosition === 'right' ? 'order-4' : 'order-1'
        ]"
      >
        <AIReviewPanel :context="appStore.aiContext" :locale="locale" />
        <!-- AI Resizer -->
        <div
      ...
      >
        ...
        >
          ...
          <div
            v-if="isGlobalLayer"
            class="flex items-center ml-2 pl-2 border-l border-gray-200 dark:border-gray-700 shrink-0"
          >
            <button
        ...
        <div class="flex-1 flex flex-col overflow-hidden relative">
          ...
          <main
            ...
            :style="{
              height: consoleStore.isVisible ? `calc(100% - ${consoleStore.height}px)` : '100%'
            }"
          >
            ...
            <div class="flex flex-col">
              ...
              >
                <div class="flex items-center gap-2 min-w-0 flex-1">
                  ...
                  <div
                    class="flex-1 h-1 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden relative"
                  >
                    <div
                      ...
                      :style="{
                        width: `${Math.min(100, Math.max(0, (p.current / p.total) * 100))}%`
                      }"
                    ></div>
                  </div>
                </div>
                ...
                <span
                  class="text-[10px] font-bold uppercase tracking-widest text-primary-600 dark:text-primary-400 shrink-0 tabular-nums min-w-[80px] text-right truncate"
                >
                  {{ p.current }}/{{ p.total }}
                </span>
              </div>
              ...
              <div
                class="h-6 flex items-center justify-between px-4 bg-gray-50 dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700"
              >
                <div class="flex items-center gap-4">
                  <!-- Collapse/Normal/Expand Modes -->
                  <div
                    class="flex items-center gap-1 border-r border-gray-200 dark:border-gray-700 pr-3"
                  >
                    <button
                      @click="progressDisplayMode = 'collapsed'"
                      :class="[
                        'p-0.5 rounded transition-colors',
                        progressDisplayMode === 'collapsed'
                ...
                >
                  <component
                    :is="progressDisplayMode === 'expanded' ? ChevronUp : Layers"
                    class="w-3 h-3"
                  />
                  {{
                    progressDisplayMode === 'expanded'
                      ? 'Show Less'
                      : `+ ${progressList.length - (progressDisplayMode === 'collapsed' ? 1 : 3)} More`
                  }}
                </button>
              </div>
            </div>
          ...
          >
            <ConsoleOutput
              ...
              @clear="consoleStore.clearLogs()"
              @close="consoleStore.setVisibility(false)"
            />
          </div>
        </div>
      </div>
...
import { X, Layers, ChevronDown, ChevronUp, LayoutList, Maximize2, Sparkles } from 'lucide-vue-next'
...
const handleSidebarResize = (e: MouseEvent) => {
  if (isResizingSidebar.value) {
    const isRight = appStore.layoutSettings.sidebarPosition === 'right'
    if (isRight) {
...
const handleAiResize = (e: MouseEvent) => {
  if (isResizingAi.value) {
    const newWidth =
      appStore.layoutSettings.aiPanelPosition === 'right'
...
const sendSelectionToAi = () => {
  ...
  window.dispatchEvent(
    new CustomEvent('andb-ai-inject-selection', {
