<template>
  <div
    class="fixed inset-0 bg-primary-900 flex items-center justify-center z-50 p-6 overflow-y-auto"
  >
    ...
    <div v-if="!showSetup" class="text-center">
      ...
      <div class="animate-pulse-slow mb-16">
        <div class="flex items-center justify-center gap-4 mb-4">
          <img src="/icon.png" alt="Andb Icon" class="w-20 h-auto" />
          <h3 class="text-5xl font-black text-white tracking-tighter">TheAndb</h3>
        </div>
        <p class="text-xl font-bold text-primary-100 tracking-tight opacity-80">
      ...
      <div class="absolute bottom-8 left-0 right-0 text-center space-y-2">
        <p class="text-xs text-primary-300 font-medium">Version {{ version }}</p>
        <div class="flex flex-col items-center justify-center space-y-1">
          <p class="text-xs text-primary-400 opacity-70">© 2024 - 2026 {{ author }}</p>
    ...
    <div
      v-else
      class="w-full max-w-4xl bg-white dark:bg-gray-900 rounded-[2rem] shadow-2xl overflow-hidden animate-fade-in flex flex-col md:flex-row shadow-primary-900/50"
    >
      <!-- Left Branding Panel -->
      <div
        class="md:w-5/12 bg-primary-500 p-8 lg:p-12 text-white flex flex-col justify-between hidden md:flex relative overflow-hidden"
      >
        <div
          class="absolute -right-20 -top-20 w-64 h-64 bg-primary-400 rounded-full mix-blend-multiply filter blur-3xl opacity-50 animate-pulse-slow"
        ></div>
        <div
          class="absolute -left-20 -bottom-20 w-64 h-64 bg-primary-600 rounded-full mix-blend-multiply filter blur-3xl opacity-50 animate-pulse-slow"
        ...
        <div class="relative z-10">
          <img
            src="/icon.png"
            alt="Logo"
            class="w-16 h-16 mb-8 brightness-0 invert drop-shadow-md"
          />
          ...
          <p class="text-sm text-primary-100 font-medium leading-relaxed opacity-90">
            The local-first intelligent database architect. Fast, native GUI for complete version
            control.
          </p>
        </div>
        ...
        <div class="relative z-10 space-y-5 pt-10 mt-auto border-t border-primary-400/50">
          <div class="flex items-center gap-4">
            <div
              class="w-10 h-10 rounded-xl bg-primary-400/30 flex items-center justify-center text-white shrink-0 shadow-inner border border-primary-400/50"
            >
              <Database class="w-5 h-5" />
            </div>
            <div>
              <div class="text-[11px] font-black uppercase tracking-widest mb-1">
                Local Metadata
              </div>
              <div class="text-[10px] text-primary-100 opacity-80">
                Safe & isolated from internet
              </div>
            </div>
          </div>
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <div class="text-[10px] text-primary-100 opacity-80">
                Connect to your local repository
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="md:w-7/12 p-8 lg:p-12 flex flex-col">
        <div class="mb-8">
          ...
          <p class="text-xs text-gray-500 font-medium leading-relaxed">
            Specify where TheAndb should securely store internal settings and where your SQL
            definitions reside before continuing.
          </p>
        </div>
        ...
        <div class="space-y-6 flex-1">
          <!-- Workspace Setup -->
          <div
            class="bg-gray-50 dark:bg-gray-800/40 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 relative group hover:border-primary-500/50 transition-colors"
          >
            <h4
              class="text-sm font-black text-gray-900 dark:text-white uppercase tracking-widest mb-1 flex items-center gap-2"
            >
              <FolderGit2 class="w-4 h-4 text-primary-500" />
              Workspace Vault Directory
            </h4>
            <p
              class="text-[11px] text-gray-500 dark:text-gray-400 mb-4 tracking-tight leading-relaxed"
            >
              Select a root folder where your SQL definitions, SQLite metadata, and security keys
              will be centrally stored. Select an existing vault to
              <strong class="text-primary-600 dark:text-primary-400">Link Existing Data</strong>, or
              an empty one to <strong>Start Fresh</strong>.
            </p>
            <div class="flex items-center gap-3">
              <div
                class="flex-1 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700/50 rounded-xl px-4 py-3 text-[10px] font-mono font-bold text-gray-500 truncate shadow-inner h-[46px] flex items-center"
              >
            ...
            <div
              v-if="workspaceMsg"
              class="absolute -top-3 right-6 bg-green-500 text-white text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest animate-fade-in shadow-sm"
            >
              <span class="flex items-center gap-1"
                ><Check class="w-3 h-3" /> {{ workspaceMsg }}</span
              >
            </div>
          </div>
        </div>
