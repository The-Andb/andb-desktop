...
const gitGuides = {
  ...
  gitlab: [
    {
      text: 'User Settings > Access Tokens',
...
const loadPaths = async () => {
  try {
    ...
    mcpBinaryPath.value =
      (await window.electronAPI?.mcp?.getMcpPath()) || '/path/to/mcp/dist/index.js'
  } catch (error) {
...
onMounted(() => {
  ...
  checkCliStatus()
  loadGitConfig()
  // Auto-fetch disabled on mount
  // setTimeout(checkGitStatus, 500)
})

watch(currentProjectId, () => {
  loadGitConfig()
  // Auto-fetch disabled on project switch
  // checkGitStatus()
})
      ...
      <div class="max-w-5xl mx-auto w-full pb-12">
        <div class="mb-8">
          ...
          <p class="text-gray-500 dark:text-gray-400">
            Version control your Schema DDL natively with Git.
          </p>
        </div>
        ...
        <div class="grid grid-cols-1 gap-8">
          ...
          <div
            class="bg-white dark:bg-gray-800 rounded-xl border border-primary-100 dark:border-primary-900/30 overflow-hidden shadow-md shadow-primary-500/5 transition-all hover:shadow-lg hover:shadow-primary-500/10"
          >
            <div
              class="p-6 border-b border-gray-200 dark:border-gray-700 bg-primary-50/30 dark:bg-primary-900/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
            >
              <div class="flex items-center gap-3">
                <div
                  class="w-10 h-10 rounded-lg bg-primary-100 dark:bg-primary-900/50 flex items-center justify-center text-primary-600 dark:text-primary-400 shrink-0"
                >
                  <GitBranch class="w-5 h-5" />
                </div>
                <div>
                  <h2 class="text-lg font-bold text-gray-900 dark:text-white">
                    Git Versioning (Hybrid)
                  </h2>
                  <p class="text-sm text-gray-500 dark:text-gray-400">
                    Mirror SQLite state to human-readable SQL files in Git.
                  </p>
                </div>
              </div>
              ...
              <div class="flex items-center gap-3">
                <div
                  class="flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium border whitespace-nowrap"
                  :class="
                    gitStatus?.isRepo
            ...
            <div class="p-6">
              <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                ...
                <div class="space-y-4">
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="space-y-1">
                      ...
                      <input
                        v-model="gitConfig.workBranch"
                        type="text"
                        placeholder="andb-work/user"
                        class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary-500 outline-none transition-all"
                      />
                    </div>
                    <div class="space-y-1">
                  ...
                  <div class="pt-4 border-t border-gray-100 dark:border-gray-700 space-y-4">
                      ...
                      <div class="space-y-1">
                        <div class="flex items-center justify-between">
                          ...
                          <button
                            ...
                            class="text-[10px] font-bold text-primary-500 hover:text-primary-600 flex items-center gap-1 transition-colors"
                          >
                    ...
                    <div
                      v-if="showTokenGuide"
                      class="p-4 bg-primary-50/50 dark:bg-primary-900/10 border border-primary-100 dark:border-primary-900/30 rounded-xl space-y-4 animate-in slide-in-from-top-2 duration-200"
                    >
                      <div
                        class="flex items-center justify-between border-b border-primary-100 dark:border-primary-900/30 pb-2"
                      >
                        <h4
                          class="text-xs font-black text-primary-700 dark:text-primary-400 uppercase tracking-widest"
                        >
                          Git PAT Instructions
                        </h4>
                        <button
                          @click="showTokenGuide = false"
                          class="p-1 hover:bg-primary-100 dark:hover:bg-primary-900/30 rounded"
                        >
                      ...
                      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                          <div
                            class="flex items-center gap-2 text-[10px] font-bold text-gray-600 dark:text-gray-300"
                          >
                            <span
                              class="w-4 h-4 rounded-full bg-black text-white flex items-center justify-center text-[8px]"
                              >GH</span
                            >
                            GITHUB
                          </div>
                          <ul class="space-y-1.5">
                            <li
                              v-for="(step, idx) in gitGuides.github"
                              :key="idx"
                              class="text-[10px] text-gray-500 dark:text-gray-400 flex items-start gap-1.5"
                            >
                              <span
                                class="mt-1 w-1 h-1 rounded-full bg-primary-400 shrink-0"
                              ></span>
                              <div class="flex-1">
                                {{ step.text }}
                                <a
                                  v-if="step.url"
                                  href="#"
                                  @click.prevent="openExternal(step.url)"
                                  class="text-primary-500 hover:underline inline-flex items-center ml-1"
                                >
                        ...
                        <div class="space-y-2">
                          ...
                          >
                            <span
                              class="w-4 h-4 rounded-full bg-orange-500 text-white flex items-center justify-center text-[8px]"
                              >GL</span
                            >
                            GITLAB
                          </div>
                          <ul class="space-y-1.5">
                            <li
                              v-for="(step, idx) in gitGuides.gitlab"
                              :key="idx"
                              class="text-[10px] text-gray-500 dark:text-gray-400 flex items-start gap-1.5"
                            >
                              <span
                  ...
                  <div
                    class="flex flex-col xl:flex-row items-stretch xl:items-center justify-between gap-4 pt-6 mt-2 border-t border-gray-100 dark:border-gray-800"
                  >
                    <div class="flex items-center gap-4">
                      <button
                        @click="saveGitConfig"
                        class="flex items-center justify-center gap-2 px-5 py-3 bg-gray-900 dark:bg-white dark:text-gray-900 text-white rounded-xl text-sm font-bold hover:shadow-lg hover:shadow-gray-500/20 active:scale-[0.98] transition-all duration-200"
                      >
                      ...
                      <label class="flex items-center gap-2 cursor-pointer group">
                        <div class="relative">
                          <input
                            type="checkbox"
                            v-model="gitConfig.autoCommit"
                            class="sr-only peer"
                          />
                          <div
                            class="w-10 h-5 bg-gray-200 dark:bg-gray-700 rounded-full peer peer-checked:bg-primary-500 transition-colors"
                          ></div>
                          <div
                        ...
                        <span
                          class="text-xs font-bold text-gray-500 group-hover:text-gray-700 dark:group-hover:text-gray-300 transition-colors uppercase tracking-tight"
                          >Auto-Commit</span
                        >
                      </label>
                    </div>
                  ...
                  <div
                    v-if="isPreviewing"
                    class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-in fade-in duration-200"
                  >
                    <div
                      class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200"
                    >
                      <div
                      ...
                      >
                        <div class="flex items-center gap-3">
                          <div
                            class="w-10 h-10 rounded-xl bg-primary-100 dark:bg-primary-900/50 flex items-center justify-center text-primary-600 dark:text-primary-400"
                          >
                            <GitBranch class="w-5 h-5" />
                          </div>
                          <div>
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400">
                              Semantic message generated from AST diff.
                            </p>
                          </div>
                        </div>
                        <div
                          class="text-xs font-mono bg-orange-100 dark:bg-orange-900/30 text-orange-600 px-2 py-1 rounded"
                        >
                      ...
                      <div class="p-6 space-y-4">
                        <div class="space-y-2">
                          ...
                          <textarea
                            ...
                            rows="3"
                            class="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary-500 outline-none transition-all"
                          ></textarea>
                        </div>
                      ...
                      <div class="p-4 bg-gray-50 dark:bg-gray-900/50 flex gap-3">
                        <button
                          ...
                          class="flex-1 px-4 py-2.5 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 rounded-xl text-sm font-bold hover:bg-gray-50 transition-colors"
                        >
                          Cancel
                        </button>
                        <button
                          @click="confirmGitSync"
                          class="flex-1 px-4 py-2.5 bg-primary-600 text-white rounded-xl text-sm font-bold hover:bg-primary-700 shadow-lg shadow-primary-600/20 transition-all"
                        >
                          Confirm & Sync
                        </button>
                      </div>
                ...
                <!-- Info/Status -->
                <div
                  class="bg-gray-50 dark:bg-gray-900/50 rounded-xl p-6 border border-gray-100 dark:border-gray-800"
                >
                  ...
                  <ul class="space-y-3">
                    <li class="flex gap-3 text-sm text-gray-600 dark:text-gray-400">
                      <div
                        class="shrink-0 w-5 h-5 rounded-full bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 flex items-center justify-center text-[10px] font-bold"
                      >
                        1
                      </div>
                      <div>
                        <strong>Real Diffs:</strong> Each database object is stored as an individual
                        <code>.sql</code> file, resulting in perfect Git history.
                      </div>
                    </li>
                    <li class="flex gap-3 text-sm text-gray-600 dark:text-gray-400">
                      <div
                      ...
                      <div>
                        <strong>Zero Lag:</strong> SQLite handles the UI operations instantly, while
                        Git manages the long-term proof of work.
                      </div>
                    </li>
                    <li class="flex gap-3 text-sm text-gray-600 dark:text-gray-400">
                      <div
                      ...
                      <div>
                        <strong>Team Sync:</strong> Push your schema changes to a shared repository
                        for code reviews and team parity.
                      </div>
                    </li>
                  </ul>

                  <div
                    v-if="gitStatus?.behind > 0"
                    class="mt-4 p-4 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-xl flex gap-3 animate-pulse"
                  >
                    <AlertCircle class="w-5 h-5 text-orange-600 shrink-0" />
                    <div>
                      <h4 class="text-sm font-bold text-orange-800 dark:text-orange-400">
                        Drift Detected (Behind Remote)
                      </h4>
                      <p class="text-xs text-orange-700 dark:text-orange-500 mt-1">
                        Git remote is ahead by <strong>{{ gitStatus.behind }}</strong> commits. We
                        recommend pulling changes before making new local exports to avoid
                        conflicts.
                      </p>
                    </div>
                  </div>
                  ...
                  <div
                    v-if="gitStatus"
                    class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 space-y-3"
                  >
                    <div class="flex justify-between text-sm">
                      <span class="text-gray-500">Current Branch:</span>
                      <span class="font-mono text-primary-600 font-bold tracking-tight">{{
                    ...
                    <div class="flex justify-between text-sm">
                      ...
                      <span
                        class="font-bold"
                        :class="
                          gitStatus.modifiedFiles?.length > 0 ? 'text-orange-500' : 'text-green-500'
                        "
                      >
                    ...
                    <div class="flex justify-between text-sm">
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <span
                          v-if="gitStatus.ahead > 0"
                          class="px-2 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded text-[10px] font-bold"
                          >Ahead: {{ gitStatus.ahead }}</span
                        >
                        <span
                          v-if="gitStatus.behind > 0"
                          class="px-2 py-0.5 bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 rounded text-[10px] font-bold"
                          >Behind: {{ gitStatus.behind }}</span
                        >
                        <span
