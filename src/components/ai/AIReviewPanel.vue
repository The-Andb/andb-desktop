...
const personas = [
  {
    ...
    prompt:
      'You are an experienced Database Administrator. Focus on database performance, best practices, and schema design. IMPORTANT: NEVER mention internal source code, file names, or internal implementation details of TheAndb app. Focus only on the database logic and user-facing features.'
  },
  {
    id: 'secretary',
    name: 'Secretary',
    ...
    prompt:
      'You are a professional secretary for TheAndb. You help users navigate the app and summarize database information. NEVER show code or internal paths. Focus on guiding the user through the UI and explaining features.'
  },
  {
    id: 'coworker',
    name: 'Co-worker',
    ...
    prompt:
      'You are a helpful co-worker. You use casual language ("bro", "ê", "ông"). Help the user solve database problems and explain how to use TheAndb features. STRICT RULE: Do not disclose file paths, source code, or internal logic. Talk like a user-facing support engineer, not a core developer.'
  },
  {
    id: 'playful',
    name: 'Playful',
    ...
    prompt:
      'You are a witty AI assistant. Use humor and fun analogies. Focus on helping the user with their database tasks. NEVER mention internal code or dev details. Keep the magic hidden!'
  }
] as const
...
const selectedPersonaId = computed({
  ...
  set: val => {
    settingsStore.settings.aiPersona = val
  }
})
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <h3
          class="text-xs font-black uppercase tracking-widest text-gray-900 dark:text-white flex items-center gap-2"
        >
      ...
      <div class="flex-1 overflow-y-auto p-2 custom-scrollbar">
        ...
        >
          <div class="overflow-hidden flex-1">
            ...
            <div
              class="text-[9px] font-medium text-gray-400 mt-1 flex items-center gap-1 uppercase tracking-wider"
            >
    ...
    <div
      v-if="showHistory"
      @click="showHistory = false"
      class="absolute inset-0 bg-gray-900/10 dark:bg-black/20 backdrop-blur-sm z-20"
    ></div>
    ...
    <div class="flex-1 flex flex-col h-full w-full bg-white dark:bg-gray-900 relative z-10">
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3
              class="text-xs font-bold text-gray-900 dark:text-white leading-tight uppercase tracking-wide"
            >
              {{ t('aiAssistant.title') }}
            </h3>
            <p
              v-if="context"
              class="text-[9px] text-gray-500 font-medium uppercase tracking-wider opacity-70"
            >
              {{ context.objectType }}: {{ context.objectName }}
            </p>
            <p
              v-else
              class="text-[9px] text-gray-500 font-medium uppercase tracking-wider opacity-70"
      ...
      <!-- Content Area -->
      <div
        class="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar flex flex-col ai-chat-container"
      >
        ...
        >
          ...
          <div class="space-y-2 mb-8">
            <div class="flex items-center justify-center gap-2 overflow-hidden px-4">
              <span class="text-xs font-black text-primary-500 uppercase tracking-widest">{{
                t('aiAssistant.xinChao')
              }}</span>
            ...
            >
              I'm TheAndb
              <span
                class="px-2 py-0.5 rounded-md bg-primary-500/10 text-primary-500 text-[10px] uppercase tracking-widest"
                >AI DBA</span
              >
            </h2>
            <p
          ...
          <div class="grid grid-cols-1 gap-2 w-full max-w-[240px]">
            ...
            >
              <div
                class="w-5 h-5 rounded-lg bg-white dark:bg-gray-900 flex items-center justify-center shadow-sm"
              >
        ...
        <template v-else>
          ...
          >
            ...
            <div
              :class="[
                'max-w-[90%] px-5 py-4 rounded-3xl text-[14px] leading-relaxed relative shadow-sm border',
                msg.role === 'ai'
                  ? 'bg-white dark:bg-gray-800/40 text-gray-700 dark:text-gray-200 border-gray-100 dark:border-gray-700/50'
                  : msg.role === 'error'
                    ? 'bg-red-50/50 dark:bg-red-950/10 text-red-700 dark:text-red-400 border-red-100/50 dark:border-red-900/30'
            ...
            >
              ...
              <div
                ...
                :class="[
                  ...
                  msg.role === 'ai'
                    ? 'dark:prose-invert text-gray-700 dark:text-gray-200'
                    : msg.role === 'error'
          ...
          <div v-if="isLoading" class="flex gap-3">
            ...
            >
              ...
              <span class="text-[11px] text-gray-500 font-black uppercase tracking-widest">{{
