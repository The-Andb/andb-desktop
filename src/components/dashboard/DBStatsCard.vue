<template>
  <div
    class="group bg-white dark:bg-gray-800 rounded-[2rem] p-8 border border-gray-100 dark:border-gray-700/50 shadow-sm hover:shadow-2xl transition-all duration-500 relative overflow-hidden"
  >
    <!-- Background Sparkle Deco -->
    <div
      class="absolute -right-20 -top-20 w-48 h-48 bg-primary-500/5 rounded-full blur-3xl group-hover:bg-primary-500/10 transition-colors duration-500"
    ></div>
    <div
      class="absolute -left-10 -bottom-10 w-32 h-32 bg-indigo-500/5 rounded-full blur-2xl group-hover:bg-indigo-500/10 transition-colors duration-500"
    ></div>
    ...
    <div class="relative z-10">
      <div class="flex items-center justify-between mb-8">
        <div class="flex items-center gap-4">
          <div
            class="p-3 bg-primary-500/10 text-primary-500 rounded-2xl group-hover:scale-110 transition-transform duration-300 shadow-sm border border-primary-500/10 overflow-hidden relative"
          >
            <div
              class="absolute inset-0 bg-gradient-to-br from-primary-500/20 to-transparent"
            ></div>
            <Activity class="w-6 h-6 relative z-10" />
          </div>
          <div>
            <h3 class="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-0.5">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        <div
          ...
          :key="item.label"
          class="space-y-1 p-3 bg-gray-50/50 dark:bg-gray-900/50 rounded-2xl border border-gray-100/50 dark:border-gray-700/50"
        >
          ...
          <div class="flex items-baseline gap-1">
            <span class="text-lg font-black text-gray-900 dark:text-white tracking-tight">{{
              item.value
            }}</span>
      ...
      <div class="grid grid-cols-2 gap-6 mb-8">
        ...
        <div class="relative">
          ...
          <div class="flex items-baseline gap-1">
            ...
            <span class="text-[10px] font-bold text-primary-500/60 uppercase">{{
              formattedSize.unit
            }}</span>
          </div>
        </div>
      ...
      <div
        v-if="engineCounts"
        class="space-y-3 pt-6 border-t border-gray-50 dark:border-gray-700/50"
      >
        <div
          class="flex items-center justify-between text-[9px] font-black uppercase tracking-widest text-gray-400"
        >
          <span>Engine Distribution</span>
          <span class="text-primary-500">{{ totalTables }} Tables config</span>
        </div>
        <div
          class="flex h-2 w-full bg-gray-100 dark:bg-gray-700/50 rounded-full overflow-hidden shadow-inner"
        >
          <div
            v-for="(count, engine, index) in engineCounts"
            :key="engine"
            :style="{ width: (count / totalTables) * 100 + '%' }"
            :class="[
              index === 0
                ? 'bg-primary-500'
                : index === 1
      ...
      <div
        v-if="serverInfo"
        class="mt-8 pt-6 border-t border-gray-50 dark:border-gray-700/50 flex items-center justify-between"
      >
        <div class="flex items-center gap-3">
          <div
            class="w-10 h-10 rounded-xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center border border-gray-200 dark:border-gray-700 shadow-sm relative group/srv"
          >
          ...
          <div>
            ...
            <div class="text-[11px] font-black text-gray-700 dark:text-gray-300 font-mono">
        ...
        <div class="text-right">
          ...
          <div class="flex items-center gap-1.5 justify-end">
            <Zap
              v-if="serverInfo.hasInstantDDL"
              class="w-3 h-3 text-emerald-500 animate-pulse fill-emerald-500"
            />
            <div class="text-xs font-black text-primary-500 uppercase tracking-widest">
              {{ serverInfo.engine || 'Relational' }}
            </div>
          </div>
        </div>
      </div>
...
const displayStats = computed(() => {
  ...
  return [
    ...
    {
      label: 'Storage',
      value: formattedSize.value.value,
      icon: Database,
      unit: formattedSize.value.unit
    }
  ]
})
