<template>
  <div class="flex h-full w-full bg-white dark:bg-gray-900">
    ...
    <!-- Left: Project Tree -->
    <div
      class="w-80 flex flex-col border-r border-gray-200 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50"
    >
      <!-- Toolbar -->
      <div class="p-2 border-b border-gray-100 dark:border-gray-800 flex items-center gap-1">
        <div
          class="flex-1 px-2 py-1.5 bg-gray-200/50 dark:bg-gray-800 text-xs text-gray-500 rounded-lg flex items-center gap-2"
        >
          <Search class="w-3.5 h-3.5" />
          <span>Search...</span>
        </div>
        <button
          class="p-1.5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 rounded-lg hover:bg-gray-200/50 dark:hover:bg-gray-800 transition-colors"
        >
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar p-2">
        <div class="flex flex-col gap-0.5">
          <!-- Project Root Node -->
          <div
            class="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-primary-50 dark:bg-primary-900/10 text-primary-700 dark:text-primary-400 font-bold text-xs select-none cursor-pointer"
          >
          ...
          <div
            class="ml-4 flex flex-col border-l border-gray-200 dark:border-gray-800 pl-2 mt-1 gap-1"
          >
            ...
            <div class="group">
              ...
              >
                <ChevronDown
                  ...
                  :class="{ '-rotate-90': !expandedNodes.has('cat-connections') }"
                />
                <Database class="w-3.5 h-3.5 text-blue-500" />
                <span>Connections</span>
                <span class="text-gray-400 text-[9px] ml-auto">{{
                  currentProject?.connectionIds.length
                }}</span>
              </div>

              <div
                v-show="expandedNodes.has('cat-connections')"
                class="ml-5 flex flex-col border-l border-gray-200 dark:border-gray-800 pl-2 mt-1 gap-0.5"
              >
                <div v-for="conn in projectConnections" :key="conn.id">
                  ...
                  >
                    ...
                    <div
                      ...
                      :class="conn.status === 'connected' ? 'bg-green-500' : 'bg-gray-300'"
                    ></div>
                  ...
                  <div
                    v-if="expandedNodes.has(conn.id)"
                    class="ml-4 pl-2 border-l border-gray-200 dark:border-gray-800 mt-0.5"
                  >
                        ...
                        <div v-if="getDatabaseData(conn.id)[type.key]?.length > 0">
                          ...
                          >
                            <ChevronDown
                              ...
                              :class="{
                                '-rotate-90': !expandedNodes.has(`${conn.id}-${type.key}`)
                              }"
                            />
                            <component :is="type.icon" class="w-3 h-3" :class="type.color" />
                            <span>{{ type.label }}</span>
                            <span class="text-[9px] opacity-50">{{
                              getDatabaseData(conn.id)[type.key].length
                            }}</span>
                          </div>

                          <!-- Objects List -->
                          <div
                            v-if="expandedNodes.has(`${conn.id}-${type.key}`)"
                            class="ml-4 pl-2 border-l border-gray-100 dark:border-gray-800"
                          >
                            <div
    ...
    <div class="flex-1 bg-white dark:bg-gray-900 flex flex-col">
      <div v-if="selectedNode" class="flex-1 flex flex-col">
        <!-- Context Header -->
        <div
          class="h-14 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between px-6"
        >
          <div class="flex items-center gap-3">
            ...
            <span
              class="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 text-[10px] font-bold uppercase text-gray-500 rounded"
          ...
          <div>
            <!-- Actions -->
            <button
              class="px-3 py-1.5 bg-primary-600 hover:bg-primary-700 text-white text-xs font-bold rounded-lg uppercase tracking-wider"
            >
              Edit
            </button>
          </div>
        ...
        <div class="flex-1 p-0 overflow-hidden relative">
          <div
            v-if="isFetchingDDL"
            class="absolute inset-0 flex items-center justify-center bg-white/80 dark:bg-gray-900/80 z-10 backdrop-blur-sm"
          >
          ...
          <div v-else class="p-6 h-full overflow-y-auto">
            <div
              class="p-6 border border-dashed border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-800/50 text-center"
            >
              <p class="text-sm text-gray-500">
                Details for {{ selectedNode.data.name }} ({{ selectedNode.type }})
              </p>
      ...
      <div
        v-else
        class="flex-1 flex flex-col items-center justify-center text-gray-300 dark:text-gray-700"
      >
