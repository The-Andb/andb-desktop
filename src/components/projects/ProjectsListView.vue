...
const getValidConnectionCount = (project: any) => {
  ...
  return project.connectionIds.filter((id: string) => appStore.connections.some(c => c.id === id))
    .length
}
...
const getValidPairCount = (project: any) => {
  ...
  for (let i = 0; i < sortedEnvs.length - 1; i++) {
    ...
    const hasCustom = connectionPairsStore.connectionPairs.some(
      p => project.pairIds?.includes(p.id) && p.sourceEnv === source.id && p.targetEnv === target.id
    )
...
<template>
  <div
    class="h-full flex flex-col bg-gray-50/30 dark:bg-gray-900/30 p-8 overflow-y-auto custom-scrollbar"
    @click="closeMenu"
  >
    <!-- Bulk Delete Modal -->
    ...
    <!-- Header Area -->
    <div
      class="max-w-7xl mx-auto w-full mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6"
    >
      <div class="space-y-2">
        ...
        <p
          class="text-gray-500 dark:text-gray-400 font-medium tracking-wide flex items-center gap-2"
        >
          ...
          >
            {{ projectsStore.projects.length }} PROJECT{{
    ...
    >
      <div class="flex items-center gap-3 border-r border-white/10 pr-6">
        ...
        >
          <div
            class="w-5 h-5 rounded border-2 border-primary-500 flex items-center justify-center bg-transparent"
          >
            <Check
              v-if="selectedIds.length === selectableProjects.length"
        ...
        <div
          class="w-8 h-8 rounded-full bg-primary-500 flex items-center justify-center font-black text-sm"
        >
          {{ selectedIds.length }}
        </div>
        <div class="flex flex-col">
          ...
          <span class="text-xs font-bold"
            >{{ selectedIds.length }} Project{{ selectedIds.length > 1 ? 's' : '' }}</span
          >
        </div>
      </div>
    ...
    <div class="max-w-7xl mx-auto w-full pb-20">
      ...
      >
        ...
        >
          ...
          <div
            v-if="project.isProtected"
            class="absolute top-0 left-0 w-24 h-24 overflow-hidden rounded-tl-[2.5rem] pointer-events-none z-50"
          >
            <div
              class="absolute top-[18px] left-[-28px] w-[120px] bg-amber-500 text-white text-[8px] font-black py-1 shadow-lg transform -rotate-45 flex items-center justify-center gap-1.5 tracking-[0.2em] uppercase"
            >
          ...
          <div
            class="absolute inset-0 bg-[var(--project-accent)]/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none z-0"
          ></div>

          <!-- Selection Checkbox (Grid) -->
          <div
            v-if="project.name !== 'TheAndb System'"
            class="absolute top-5 left-5 z-40 transition-all duration-300 opacity-100 scale-100"
          >
            <button
          ...
          <!-- Glow Effect -->
          <div
            class="absolute -right-20 -top-20 w-64 h-64 bg-[var(--project-accent)] opacity-[0.03] group-hover:opacity-[0.08] rounded-full blur-[80px] transition-opacity duration-1000"
          ></div>
          ...
          <div class="flex items-start justify-between mb-8 relative z-30">
            ...
            <div class="relative">
              ...
              <!-- Menus -->
              <div
                v-if="activeIconPickerId === project.id"
                class="absolute left-0 top-full mt-2 z-[60]"
              >
                <ProjectIconPicker
          ...
          <div class="space-y-2 mb-8 relative z-10">
            ...
            >
              ...
              <span
                ...
                class="w-2.5 h-2.5 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)] animate-pulse"
              ></span>
            </h3>
            ...
            <p
              class="text-sm text-gray-500 dark:text-gray-400 font-medium line-clamp-2 min-h-[3rem] tracking-tight leading-relaxed"
            >
          ...
          >
            ...
            >
              ...
              <span class="text-gray-900 dark:text-white">{{
                getValidConnectionCount(project)
              }}</span>
            </div>
            <div
              class="flex items-center gap-2 bg-gray-100/50 dark:bg-gray-900/50 px-4 py-2 rounded-2xl border border-gray-200/50 dark:border-gray-700/50 group-hover:bg-white dark:group-hover:bg-gray-800 transition-all duration-300"
          ...
          >
            <div class="flex flex-col gap-1">
              <div
                v-if="project.lastOpenedAt"
                class="flex items-center gap-1.5 text-[var(--project-accent)] font-black"
              >
        ...
        >
          ...
          <div class="text-center group-hover:translate-y-1 transition-transform">
            <div class="text-sm font-black text-gray-900 dark:text-white tracking-widest mb-1">
              Add New Project
            </div>
            <div class="text-xs font-bold text-gray-500 dark:text-gray-400 tracking-wide">
              Start a fresh project cluster
            </div>
          </div>
        </div>
        ...
        >
          ...
          <div class="text-center group-hover:translate-y-1 transition-transform">
            ...
            <div class="text-xs font-bold text-orange-500/70 tracking-wide">
              Try Andb with sample SQL dumps
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-else class="flex flex-col gap-4 animate-in fade-in duration-500">
        ...
        >
          ...
          <div
            v-if="project.isProtected"
            class="absolute top-0 left-0 w-20 h-20 overflow-hidden rounded-tl-3xl pointer-events-none z-50"
          >
            <div
              class="absolute top-[14px] left-[-24px] w-[100px] bg-amber-500 text-white text-[7px] font-black py-0.5 shadow-lg transform -rotate-45 flex items-center justify-center gap-1 tracking-[0.2em] uppercase"
            >
          ...
          <div
            v-if="project.name !== 'TheAndb System'"
            class="shrink-0 w-7 z-40 transition-all duration-300 opacity-100"
          >
            <button
              @click.stop="toggleSelection(project.id)"
              class="w-7 h-7 rounded-xl border-2 flex items-center justify-center transition-all bg-white dark:bg-gray-800"
          ...
          <div
            v-if="activeIconPickerId === project.id"
            class="absolute left-20 top-full mt-1 z-[60]"
          >
            <ProjectIconPicker
              :selected-icon="project.icon"
              :selected-color="project.color"
          ...
          <div
            class="flex-1 min-w-0 relative z-10 grid grid-cols-1 md:grid-cols-12 gap-6 items-center"
          >
            <!-- Name & Desc -->
            <div class="md:col-span-5 space-y-1">
              ...
              >
                ...
                <span
                  v-if="project.isActive"
                  class="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_5px_rgba(34,197,94,0.5)] animate-pulse"
                ></span>
                <span
                  v-if="defaultCliProjectId === project.id"
                  title="CLI Default Target"
              ...
              <p
                class="text-xs text-gray-500 dark:text-gray-400 font-medium truncate tracking-tight"
              >
            ...
            <div class="md:col-span-3 flex items-center justify-between">
              <div class="flex flex-col text-left">
                <span
                  v-if="project.lastOpenedAt"
                  class="text-[9px] font-black text-[var(--project-accent)] tracking-widest mb-1"
                  >Last Open</span
                >
                <span v-else class="text-[9px] font-black text-gray-400 tracking-widest mb-1"
                  >Last Update</span
                >
                <span
                  class="text-[10px] font-bold text-gray-500 dark:text-gray-400 flex items-center gap-2"
                >
                  <component
                    :is="project.lastOpenedAt ? Play : Clock"
                    class="w-3.5 h-3.5"
                    :class="{ 'fill-current': !!project.lastOpenedAt }"
                  />
