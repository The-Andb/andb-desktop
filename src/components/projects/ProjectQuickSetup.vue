<template>
  <div class="space-y-8 animate-in fade-in duration-500">
    <!-- Intro -->
    <div
      class="flex items-start gap-4 p-4 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-2xl"
    >
      <div
        class="p-2 bg-indigo-100 dark:bg-indigo-800 rounded-lg text-indigo-600 dark:text-indigo-300"
      >
        <Wand2 class="w-5 h-5" />
      </div>
      <div>
        <h3
          class="text-sm font-bold text-indigo-900 dark:text-indigo-100 uppercase tracking-wide mb-1"
        >
    ...
    <div class="space-y-4">
      <div class="flex items-center gap-2 pb-2 border-b border-gray-100 dark:border-gray-800">
        <div
          class="w-6 h-6 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-xs font-bold text-gray-500"
        >
          1
        </div>
        ...
        >
          <div class="flex items-center gap-4">
            <div class="flex items-center gap-2">
              <span
                class="text-xs font-bold px-2 py-1 rounded bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300"
              >
                {{ pair.source }}
              </span>
...
const availableEnvironments = computed(() =>
  connectionPairsStore.environments.sort((a, b) => a.order - b.order)
)
...
const applySetup = async () => {
  ...
  try {
    ...
    for (const env of selectedEnvs.value) {
      ...
      const existing = appStore.resolvedConnections.find(
        c => c.name === env && c.environment === env
      )
      if (existing) {
    ...
    for (const pair of suggestedPairs.value) {
      ...
      if (sourceId && targetId) {
        // Check existing
        const existingPair = connectionPairsStore.connectionPairs.find(
          p => p.sourceEnv === pair.source && p.targetEnv === pair.target
        )
