...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
    ...
    <!-- Modal Content -->
    <div
      class="relative bg-white dark:bg-gray-900 w-full max-w-2xl rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-800 overflow-hidden animate-in zoom-in-95 duration-200"
    >
      <!-- Header -->
      <div
        class="px-8 py-6 border-b border-gray-50 dark:border-gray-800 flex items-center justify-between bg-gray-50/50 dark:bg-gray-800/50"
      >
        <div>
          <h2 class="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight">
            {{ $t('common.quickDump.title') }}
          </h2>
          <p class="text-xs text-gray-500 font-medium uppercase tracking-widest mt-1">
            {{ $t('common.quickDump.subtitle') }}
          </p>
        </div>
        <button
          @click="emit('close')"
          class="p-2 hover:bg-white dark:hover:bg-gray-700 rounded-xl transition-colors text-gray-400"
        >
          <X class="w-5 h-5" />
        </button>
      </div>
      ...
      <div class="p-8 space-y-8">
        ...
        <div class="space-y-2">
          <label
            ...
            >{{ $t('common.quickDump.projectNameLabel') }}</label
          >
          <input
            v-model="projectName"
            type="text"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
          <!-- Connector Icon -->
          <div
            class="absolute left-1/2 top-12 -translate-x-1/2 hidden md:flex w-10 h-10 rounded-full bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 items-center justify-center z-10 shadow-sm"
          >
          ...
          <div class="space-y-3">
            <label
              class="block text-[10px] font-black text-indigo-500 uppercase tracking-widest ml-1 flex items-center gap-2"
            >
            ...
            >
              <div v-if="!sourcePath" class="text-center">
                <Upload
                  class="w-8 h-8 text-indigo-400 mx-auto mb-1 group-hover:scale-110 transition-transform"
                />
                <span class="text-[10px] font-black text-indigo-400 uppercase tracking-widest">{{
                  $t('common.quickDump.selectFile')
                }}</span>
              </div>
              <div v-else class="text-center px-4">
                <FileCode class="w-8 h-8 text-indigo-500 mx-auto mb-1" />
                <span
                  class="text-xs font-bold text-gray-700 dark:text-gray-300 truncate block w-full"
                  >{{ getFileName(sourcePath) }}</span
                >
              </div>
            </div>
          </div>
          ...
          <div class="space-y-3">
            <label
              class="block text-[10px] font-black text-emerald-500 uppercase tracking-widest ml-1 flex items-center gap-2"
            >
            ...
            >
              <div v-if="!targetPath" class="text-center">
                <Upload
                  class="w-8 h-8 text-emerald-400 mx-auto mb-1 group-hover:scale-110 transition-transform"
                />
        ...
        <div
          v-if="error"
          class="p-4 bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 rounded-2xl flex items-start gap-3"
        >
          <AlertCircle class="w-5 h-5 text-red-500 shrink-0 mt-0.5" />
          <p class="text-xs font-bold text-red-600 dark:text-red-400 uppercase tracking-wide">
            {{ error }}
          </p>
        </div>
      </div>
      ...
      <!-- Actions -->
      <div
        class="px-8 py-6 bg-gray-50/50 dark:bg-gray-800/50 border-t border-gray-50 dark:border-gray-800 flex items-center justify-end gap-4"
      >
        <button
          @click="emit('close')"
