<template>
  <Transition name="fade">
    <div v-if="isOpen" class="fixed inset-0 z-[1000] flex items-center justify-center p-4">
      ...
      <!-- Modal Content -->
      <div
        class="relative w-full max-w-2xl bg-white dark:bg-gray-900 rounded-[2.5rem] shadow-2xl shadow-primary-500/10 border border-gray-100 dark:border-gray-800 overflow-hidden animate-zoom-in"
      >
        <!-- Decoration -->
        <div
          class="absolute -right-20 -top-20 w-64 h-64 bg-primary-500/5 rounded-full filter blur-3xl"
        ></div>
        <div
          class="absolute -left-20 -bottom-20 w-64 h-64 bg-indigo-500/5 rounded-full filter blur-3xl"
        ></div>
        ...
        <div class="p-8 pb-4 flex items-center justify-between relative z-10">
          <div class="flex items-center gap-4">
            <div
              class="w-12 h-12 rounded-2xl bg-primary-500 flex items-center justify-center text-white shadow-lg shadow-primary-500/20"
            >
            ...
            <div>
              ...
              <p class="text-[11px] text-gray-400 font-bold uppercase tracking-widest">
                {{ $t('compare.instantAnalysis') || 'Instant engine-to-engine analysis' }}
              </p>
            </div>
          </div>
          <button
        ...
        <div class="p-8 pt-4 space-y-8 relative z-10">
          ...
          <div class="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] items-center gap-6">
            ...
            <div class="space-y-3">
              ...
              <div
                v-if="sourceConn"
                class="flex items-center gap-2 px-3 py-1.5 bg-primary-50 dark:bg-primary-900/10 rounded-lg border border-primary-100 dark:border-primary-900/30"
              >
                <span
                  class="text-[9px] font-black text-primary-600 dark:text-primary-400 uppercase tracking-widest"
                  >{{ sourceConn.type }}</span
                >
                <span v-if="sourceConn.schema" class="text-[9px] font-bold text-gray-400 uppercase"
                  >/ {{ sourceConn.schema }}</span
                >
              </div>
            </div>
            ...
            <div class="flex justify-center pt-6">
              <div
                class="w-10 h-10 rounded-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex items-center justify-center"
              >
            ...
            <div class="space-y-3">
              ...
              >
                <span
                  class="text-[9px] font-black text-primary-600 dark:text-primary-400 uppercase tracking-widest"
                  >{{ targetConn.type }}</span
                >
                <span v-if="targetConn.schema" class="text-[9px] font-bold text-gray-400 uppercase"
                  >/ {{ targetConn.schema }}</span
                >
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center justify-between pt-4">
            <div class="flex items-center gap-2">
              <div
                ...
                :class="canCompare ? 'bg-green-500' : 'bg-gray-300'"
              ></div>
              <span
                class="text-[10px] font-black uppercase tracking-widest"
                :class="canCompare ? 'text-gray-600' : 'text-gray-400'"
              >
...
const canCompare = computed(
  () => !!sourceId.value && !!targetId.value && sourceId.value !== targetId.value
)
...
const startCompare = async () => {
  ...
  recentPresets.value = [
    preset,
    ...recentPresets.value.filter(
      p => p.sourceId !== sourceId.value || p.targetId !== targetId.value
    )
  ].slice(0, 3)
