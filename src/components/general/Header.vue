  ...
  >
    <div class="flex items-center justify-between gap-4 h-full">
      ...
      <div class="flex items-center space-x-2 min-w-0 h-full">
        <!-- Logo & Brand -->
        <div
          class="flex items-center space-x-2 cursor-pointer shrink-0 transition-opacity duration-300"
        ...
        >
          <!-- Visible Label -->
          <div
            class="flex items-center justify-between w-full text-gray-900 dark:text-gray-100 transition-colors"
          >
            <div class="flex items-center min-w-0">
              ...
              >
                <component
                  :is="projectIconMap[currentProject.icon || 'Database']"
                  class="w-3 h-3"
                />
              </div>
            ...
            <ChevronDown
              class="w-3.5 h-3.5 text-gray-400 group-hover:text-primary-500 transition-all ml-2 shrink-0"
          ...
          <div
            ...
            @click.stop="isProjectPickerOpen = false"
          ></div>
          <div
            v-if="isProjectPickerOpen"
          ...
          >
            <div class="space-y-1">
              <button
                ...
                :class="
                  projectsStore.selectedProjectId === p.id
              ...
              >
                ...
                <div class="flex flex-col items-start min-w-0 text-left">
                  <span class="text-xs font-black tracking-tight truncate w-full">{{
                    p.name
                  }}</span>
                  <span v-if="p.lastOpenedAt" class="text-[9px] font-bold opacity-60 tracking-wider"
                    >Opened {{ formatDate(p.lastOpenedAt) }}</span
                  >
                </div>
      ...
      <div class="flex items-center space-x-1 shrink-0 h-full">
        ...
        >
          ...
          <span
            class="absolute top-0 right-0 w-2 h-2 rounded-full bg-red-500 border-2 border-white dark:border-gray-800"
          ></span>
        </button>
...
onMounted(() => {
  ...
  if (
    ...
    connectionPairsStore.activePair
  ) {
  ...
  if (
    ['/compare'].includes(route.path) &&
    !connectionPairsStore.selectedPairId &&
    availablePairs.value.length > 0
  ) {
...
watch(
  ...
  newPath => {
    if (
      ['/schema', '/history'].includes(newPath) &&
      !appStore.selectedConnectionId &&
      connectionPairsStore.activePair
    ) {
      appStore.selectedConnectionId = connectionPairsStore.activePair.source?.id || ''
