    ...
    <div class="flex items-center justify-between gap-4 h-full">
      ...
      <div class="flex items-center space-x-2 min-w-0 h-full">
        ...
        <div class="flex items-center space-x-2 cursor-pointer shrink-0 transition-opacity duration-300"
          ...
          <img src="/icon.png" alt="TheAndb" class="rounded shadow-sm transition-all duration-300 w-6 h-6" />
          <span
            class="font-black text-gray-900 dark:text-white hidden md:block tracking-tight transition-all duration-300 text-base">TheAndb</span>
        </div>
        ...
        <div v-if="route.path !== '/settings'"
          ...
          <div class="flex items-center justify-between w-full text-gray-900 dark:text-gray-100 transition-colors">
            ...
            <ChevronDown class="w-3.5 h-3.5 text-gray-400 group-hover:text-primary-500 transition-all ml-2 shrink-0"
              :class="{ 'rotate-180': isProjectPickerOpen }" />
          </div>

          <!-- Subtitle -->
          <span
            class="text-[8px] font-black text-gray-400 dark:text-gray-500 tracking-widest leading-none ml-[28px] mt-0.5 opacity-60 text-left">
            Project context
          </span>
          ...
          <div v-if="isProjectPickerOpen"
            ...
            <div class="space-y-1">
              <button v-for="p in projectsStore.projects" :key="p.id" @click.stop="selectProject(p.id)"
                ...
                <div class="flex flex-col items-start min-w-0 text-left">
                  ...
                  <span v-if="p.lastOpenedAt" class="text-[9px] font-bold opacity-60 tracking-wider">Opened {{
                    formatDate(p.lastOpenedAt) }}</span>
                </div>
              ...
              <button @click.stop="selectProject('__NEW__')"
      ...
      <div class="flex items-center space-x-1 shrink-0 h-full">
        <ThemeToggle />
        <LanguageToggle />
        ...
        <!-- App Settings -->
        <button @click="router.push('/settings')" class="p-2 rounded-lg transition-colors" :class="route.path === '/settings'
        ...
        <!-- Update Indicator -->
        <button v-if="updaterStore.status === 'downloaded' || updaterStore.status === 'available'"
          @click="handleUpdateClick" class="p-2 rounded-lg transition-all animate-pulse relative group"
          ...
          <span
            class="absolute top-0 right-0 w-2 h-2 rounded-full bg-red-500 border-2 border-white dark:border-gray-800"></span>
        </button>
