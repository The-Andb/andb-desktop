<template>
  ...
  >
    ...
    >
      ...
      <div
        class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full border border-gray-200 dark:border-gray-700"
      >
        <!-- Header -->
        <div
        ...
        >
          <h3
            class="text-lg leading-6 font-bold text-gray-900 dark:text-white flex items-center gap-2"
          >
        ...
        <div class="px-6 py-4">
          ...
          <div class="flex items-center gap-3 mb-4 text-sm">
            <span
              class="px-2 py-1 rounded bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 font-mono"
              >Current: {{ currentVersion }}</span
            >
            <ArrowRight class="w-4 h-4 text-gray-400" />
            <span
              class="px-2 py-1 rounded bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 font-mono font-bold"
              >New: {{ store.updateInfo?.version }}</span
            >
          </div>
          ...
          <div class="mb-6">
            ...
            <div
              class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-3 max-h-48 overflow-y-auto text-sm text-gray-600 dark:text-gray-400 prose dark:prose-invert prose-sm"
            >
          ...
          <div v-if="store.status === 'downloading'" class="space-y-2">
            ...
            <div class="flex justify-between text-xs text-gray-400">
              <span>{{ formatBytes(store.progress?.bytesPerSecond || 0) }}/s</span>
              <!-- Actually bytesPerSecond -->
              <span
                >{{ formatBytes(store.progress?.transferred || 0) }} /
                {{ formatBytes(store.progress?.total || 0) }}</span
              >
            </div>
          </div>
        </div>
