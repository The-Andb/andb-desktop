<template>
  <div class="space-y-6">
    ...
    <div class="space-y-4">
      ...
      >
        <div
          class="w-12 h-12 rounded-xl bg-primary-500/10 flex items-center justify-center text-primary-600 dark:text-primary-400 shrink-0 group-hover:scale-110 transition-transform"
        >
          <Download class="w-6 h-6" />
        </div>
        <div class="flex-1">
          <h3
            class="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tight mb-1 group-hover:text-primary-600 transition-colors"
          >
            {{ $t('backup.export.title') }}
          </h3>
          <p class="text-[11px] text-gray-400 dark:text-gray-500 leading-relaxed font-medium">
            Choose connections, environments, or settings to backup into a secure JSON file.
          </p>
        </div>
        <ArrowRight
      ...
      <!-- Import Action -->
      <label
        class="group relative flex items-center gap-6 p-6 rounded-2xl hover:bg-gray-100/30 dark:hover:bg-white/5 transition-all duration-300 text-left cursor-pointer border-b border-gray-100 dark:border-gray-800/40"
      >
        <div
          class="w-12 h-12 rounded-xl bg-blue-500/10 flex items-center justify-center text-blue-600 dark:text-blue-400 shrink-0 group-hover:scale-110 transition-transform"
        >
          <Upload class="w-6 h-6" />
        </div>
        <div class="flex-1">
          <h3
            class="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tight mb-1 group-hover:text-blue-600 transition-colors"
          >
            {{ $t('backup.import.title') }}
          </h3>
          <p class="text-[11px] text-gray-400 dark:text-gray-500 leading-relaxed font-medium">
            Analyze a backup file and smartly merge its contents with your current workspace.
          </p>
        </div>
        <ArrowRight
          class="w-5 h-5 text-gray-300 group-hover:text-blue-500 group-hover:translate-x-1 transition-all"
        />
      ...
      <div class="space-y-8">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-4">
            <div
              class="w-12 h-12 rounded-xl bg-purple-500/10 text-purple-600 dark:text-purple-400 flex items-center justify-center"
            >
            ...
            <div>
              ...
              <p class="text-[11px] text-gray-400 mt-0.5 font-medium">
                {{ $t('backup.history.desc') }}
              </p>
            </div>
          </div>
          <router-link
            to="/history"
            class="px-5 py-2.5 rounded-lg bg-gray-900 dark:bg-white text-white dark:text-gray-900 hover:bg-primary-500 dark:hover:bg-primary-400 text-[10px] font-black uppercase tracking-widest transition-all shadow-md active:scale-95 flex items-center gap-2"
          >
        ...
        <div
          class="flex items-center justify-between gap-6 p-4 rounded-xl hover:bg-gray-100/30 dark:hover:bg-white/5 transition-colors group"
        >
          <div class="flex items-center gap-4 flex-1 min-w-0">
            <div
              class="w-10 h-10 shrink-0 flex items-center justify-center rounded-lg bg-gray-50 dark:bg-gray-800 text-gray-400 group-hover:text-purple-500 transition-colors"
            >
              <Folder class="w-4 h-4 ml-0.5" />
            </div>
            <div class="min-w-0 flex-1">
              <span
                class="text-[9px] block font-black text-gray-400 uppercase tracking-widest mb-1"
                >{{ $t('backup.physicalPath') }}</span
              >
              <div class="flex items-center gap-2">
                <span
                  v-if="physicalBackupPath"
