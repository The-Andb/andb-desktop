<template>
  <div
    ref="scrollContainer"
    class="flex items-center bg-gray-50 dark:bg-gray-950 border-b border-gray-200 dark:border-gray-800 overflow-x-auto no-scrollbar w-full min-w-0 h-10 px-2 gap-0.5 scroll-smooth"
  >
    <div
      v-for="tab in tabs"
      :key="tab.id"
      :ref="
        el => {
          if (activeTabId === tab.id) activeTabRef = el
        }
      "
      @click="$emit('select', tab.id)"
      @contextmenu.prevent="openContextMenu($event, tab.id)"
      class="group relative flex items-center h-[34px] mt-[6px] px-2 rounded-t-lg cursor-pointer transition-all duration-200 select-none flex-none w-auto min-w-[100px] max-w-[240px] border-t border-x overflow-hidden"
      :class="
        activeTabId === tab.id
    ...
    >
      ...
      <span class="text-[11px] font-bold truncate flex-1 tracking-tight min-w-0">{{
        tab.name
      }}</span>
      ...
      <div
        ...
        class="absolute -bottom-px left-0 right-0 h-px bg-white dark:bg-gray-900 z-20"
      ></div>
    </div>
  ...
  async newId => {
    if (newId) {
      ...
      if (
        ...
        (scrollContainer.value as HTMLElement).isConnected
      ) {
