      ...
      >
        ...
        <Transition name="scale">
          ...
          >
            ...
            <div
              class="absolute top-0 left-0 w-full h-48 bg-gradient-to-br from-emerald-600 to-teal-700 opacity-10 dark:opacity-20"
            ></div>
            <div
              class="absolute -top-24 -right-24 w-64 h-64 bg-emerald-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob"
            ></div>
            <div
              class="absolute -top-24 -left-24 w-64 h-64 bg-teal-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-2000"
            ></div>
            ...
            <div class="relative z-10 px-8 py-8">
              ...
              <div class="flex items-center gap-3 mb-2">
                <div
                  class="w-10 h-10 rounded-xl bg-emerald-100 dark:bg-emerald-900/40 flex items-center justify-center"
                >
                  <Database
                    v-if="!isAppUpdate"
                ...
                <div>
                  ...
                  <p class="text-xs text-gray-500 dark:text-gray-400">
                    {{
                      isAppUpdate
                        ? `Version ${report.toVersion} is ready`
                        : 'Internal storage migrated automatically'
                    }}
                  </p>
                </div>
              </div>
              ...
              <div class="flex items-center gap-2 mb-6 mt-4">
                <span
                  class="px-2 py-1 rounded-md bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 text-xs font-mono"
                >
                  {{ report.fromVersion || '?' }}
                </span>
                <ArrowRight class="w-3 h-3 text-gray-400" />
                <span
                  class="px-2 py-1 rounded-md bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 text-xs font-mono font-bold"
                >
                  {{ report.toVersion }}
                </span>
              </div>
              ...
              <div class="space-y-2 max-h-64 overflow-y-auto pr-1 mb-6 scrollbar-thin">
                ...
                >
                  <!-- Table Header -->
                  <div class="flex items-center gap-2 cursor-pointer" @click="toggleExpand(idx)">
                    <span
                    ...
                    <span
                      class="font-mono text-sm font-semibold text-gray-800 dark:text-gray-200 flex-1"
                    >
                      {{ change.table }}
                    </span>
                    <span
                      class="text-[10px] uppercase tracking-wider font-bold"
                      :class="change.action === 'CREATED' ? 'text-green-500' : 'text-amber-500'"
                    >
                      {{ change.action }}
                    </span>
                    <ChevronDown
              ...
              <p class="text-xs text-gray-400 dark:text-gray-500 text-center mb-5">
                {{ report.changes.length }} table{{ report.changes.length !== 1 ? 's' : '' }}
