...
import { 
  ...
  Navigation, PanelLeftClose, LayoutTemplate, History
...
<template>
  <div class="relative">
    <button 
      ...
      class="p-2 rounded-xl transition-all border border-transparent flex items-center justify-center relative group"
    ...
    >
      <div class="relative">
         ...
         <div v-if="appStore.layoutSettings.sidebar && consoleStore.isVisible" class="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 bg-primary-500 rounded-full border border-white dark:border-gray-900 animate-pulse"></div>
      </div>
    </button>

    <!-- Dropdown (VS Code Inspired Premium Menu) -->
    <div v-if="showLayoutMenu" class="absolute right-0 top-full mt-3 w-80 bg-white dark:bg-gray-950 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 z-[100] overflow-hidden animate-in fade-in zoom-in-95 duration-200 text-gray-900 dark:text-white">
      <!-- Header -->
      <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-800/50 flex items-center justify-between bg-gray-50/50 dark:bg-gray-900/50">
        <div class="flex items-center gap-2">
           <LayoutDashboard class="w-3.5 h-3.5 text-primary-500" />
           <span class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400">View Layout</span>
        </div>
      ...
      <div class="p-1.5 space-y-0.5">
        ...
        <div class="px-2 py-1.5 text-[9px] font-black uppercase tracking-widest text-gray-400/60 flex items-center gap-2 mb-1">
           <span class="w-1 h-1 rounded-full bg-gray-300 dark:bg-gray-600"></span>
           Appearance Panels
        </div>

        <div class="grid grid-cols-4 gap-1 px-1.5 pb-2">
          <!-- Sidebar Toggle -->
          <button @click="appStore.layoutSettings.sidebar = !appStore.layoutSettings.sidebar" 
             ...
             <span class="text-[8px] font-black uppercase tracking-tighter" :class="appStore.layoutSettings.sidebar ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400'">Sidebar</span>
          </button>
          ...
          <button @click="consoleStore.toggleVisibility()" 
             ...
             <span class="text-[8px] font-black uppercase tracking-tighter" :class="consoleStore.isVisible ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400'">Console</span>
          </button>
          ...
          <button @click="appStore.layoutSettings.toolbar = !appStore.layoutSettings.toolbar" 
             ...
             <span class="text-[8px] font-black uppercase tracking-tighter" :class="appStore.layoutSettings.toolbar ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400'">Toolbar</span>
          </button>
          ...
          <button @click="appStore.layoutSettings.breadcrumbs = !appStore.layoutSettings.breadcrumbs" 
             ...
             <span class="text-[8px] font-black uppercase tracking-tighter" :class="appStore.layoutSettings.breadcrumbs ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400'">Bread</span>
          </button>
        </div>
        ...
        <!-- Swap Sidebar -->
        <button @click="swapSidebar" class="w-full px-3 py-2 flex items-center justify-between rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-all group/item">
          <div class="flex items-center gap-3">
             <ArrowRightLeft class="w-4 h-4 text-gray-400 group-hover/item:text-primary-500 transition-colors" />
             <span class="text-[11px] font-bold text-gray-700 dark:text-gray-300">Swap Sidebar Position</span>
          </div>
          <span class="text-[9px] font-mono text-gray-300 dark:text-gray-600 tracking-tighter opacity-0 group-hover/item:opacity-100 transition-opacity">⌥⌘S</span>
        </button>
        ...
        <div class="grid grid-cols-3 gap-1 px-1.5 pb-2">
           <button 
             v-for="style in ['full', 'minimal', 'icons'] as const" 
             :key="style"
             @click="appStore.buttonStyle = style"
             class="flex flex-col items-center gap-1.5 p-2 rounded-xl transition-all border group/btn"
             :class="appStore.buttonStyle === style 
               ? 'bg-primary-50 dark:bg-primary-900/20 border-primary-100 dark:border-primary-800 shadow-inner' 
               : 'border-transparent hover:bg-gray-100 dark:hover:bg-gray-800'"
           >
              <component 
                :is="style === 'full' ? Zap : style === 'minimal' ? MousePointer2 : Layers" 
              ...
              />
              <span class="text-[8px] font-black uppercase tracking-tighter" :class="appStore.buttonStyle === style ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400'">{{ style }}</span>
           </button>
        </div>

        <!-- Footer Actions -->
        <div class="border-t border-gray-100 dark:border-gray-800/50 pt-1 mt-1">
          <button @click="router.push('/settings?cat=interface')" class="w-full px-3 py-2 flex items-center gap-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-all group/item">
          ...
          <button @click="resetLayout" class="w-full px-3 py-2 flex items-center gap-3 rounded-xl hover:bg-red-50 dark:hover:bg-red-950/20 transition-all group/reset">
            <RotateCcw class="w-4 h-4 text-gray-400 group-hover/reset:text-red-500 transition-colors" />
            <span class="text-[11px] font-bold text-gray-700 dark:text-gray-300 group-hover/reset:text-red-600">Reset Workspace Layout</span>
          </button>
        </div>
      </div>
    </div>
