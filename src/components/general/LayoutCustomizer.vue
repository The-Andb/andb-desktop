...
const swapSidebar = () => {
  appStore.layoutSettings.sidebarPosition =
    appStore.layoutSettings.sidebarPosition === 'left' ? 'right' : 'left'
}

const swapAiPanel = () => {
  appStore.layoutSettings.aiPanelPosition =
    appStore.layoutSettings.aiPanelPosition === 'left' ? 'right' : 'left'
}
...
<template>
  ...
  >
    ...
    <button
      ...
      :class="
        showLayoutMenu
          ...
          : 'text-gray-400 hover:text-primary-600 hover:bg-white dark:hover:bg-gray-700'
      "
      title="Customize Layout"
    >
      <LayoutDashboard class="w-4 h-4 transition-transform group-hover:scale-110" />
      <div
        v-if="appStore.layoutSettings.sidebar && consoleStore.isVisible"
    ...
    <div class="flex items-center gap-1 px-0.5">
      <button
        @click="appStore.layoutSettings.sidebar = !appStore.layoutSettings.sidebar"
        class="w-7 h-7 rounded-lg transition-all duration-200 flex items-center justify-center relative group"
        :class="
          appStore.layoutSettings.sidebar
            ? 'bg-white dark:bg-gray-700 text-primary-500 shadow-sm border border-gray-100 dark:border-gray-600'
      ...
      <button
        ...
        class="w-7 h-7 rounded-lg transition-all duration-200 flex items-center justify-center relative group"
        :class="
          appStore.layoutSettings.aiPanel
            ? 'bg-white dark:bg-gray-700 text-primary-500 shadow-sm border border-gray-100 dark:border-gray-600'
            : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-200/50 dark:hover:bg-gray-700/50'
        "
    ...
    >
      ...
      >
        <div class="flex items-center gap-2">
          ...
          <span
            class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400"
            >View Layout</span
          >
        </div>
        <button
      ...
      <div class="p-1.5 space-y-0.5">
        ...
        <div class="grid grid-cols-5 gap-1 px-1.5 pb-2">
          ...
          >
            <component
              :is="appStore.layoutSettings.sidebarPosition === 'left' ? PanelLeft : PanelRight"
          ...
          >
            ...
            <span
              class="text-[8px] font-black uppercase tracking-tighter"
              :class="
                consoleStore.isVisible ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400'
              "
              >Console</span
            >
          </button>
          ...
          <!-- Toolbar Toggle -->
          <button
            @click="appStore.layoutSettings.toolbar = !appStore.layoutSettings.toolbar"
            class="flex flex-col items-center gap-1.5 p-2 rounded-xl transition-all border group/panel"
            :class="
              appStore.layoutSettings.toolbar
                ? 'bg-primary-50 dark:bg-primary-900/20 border-primary-100 dark:border-primary-800 shadow-inner'
                : 'border-transparent hover:bg-gray-100 dark:hover:bg-gray-800'
            "
            title="Toolbar (⌥⌘M)"
          ...
          <!-- Breadcrumbs Toggle -->
          <button
            @click="appStore.layoutSettings.breadcrumbs = !appStore.layoutSettings.breadcrumbs"
            class="flex flex-col items-center gap-1.5 p-2 rounded-xl transition-all border group/panel"
            :class="
              appStore.layoutSettings.breadcrumbs
                ? 'bg-primary-50 dark:bg-primary-900/20 border-primary-100 dark:border-primary-800 shadow-inner'
                : 'border-transparent hover:bg-gray-100 dark:hover:bg-gray-800'
            "
            title="Path (⌥⌘P)"
        ...
        >
          <div class="flex items-center gap-3">
            <PanelLeft
              class="w-4 h-4 text-gray-400 group-hover/item:text-primary-500 transition-colors"
            />
          ...
          <span
            class="text-[9px] font-mono text-gray-300 dark:text-gray-600 tracking-tighter opacity-0 group-hover/item:opacity-100 transition-opacity"
            >⌥⌘S</span
          >
        </button>
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <span
              class="text-[11px] font-black text-primary-600 dark:text-primary-400 uppercase tracking-tighter"
              >Swap Sidebar & AI Assistant</span
            >
          </div>
        </button>
        ...
        <div class="border-t border-gray-100 dark:border-gray-800/50 pt-1 mt-1">
          <button
            @click="router.push('/settings?cat=interface')"
          ...
          >
            ...
            />
            <span class="text-[11px] font-bold text-gray-700 dark:text-gray-300"
              >Advanced Interface Settings</span
            >
          </button>
          ...
          >
            <RotateCcw
              class="w-4 h-4 text-gray-400 group-hover/reset:text-red-500 transition-colors"
            />
            <span
              class="text-[11px] font-bold text-gray-700 dark:text-gray-300 group-hover/reset:text-red-600"
              >Reset Workspace Layout</span
            >
          </button>
        </div>
      </div>
