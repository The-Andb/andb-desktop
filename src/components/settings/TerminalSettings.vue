...
const saveDefaultCliProject = async () => {
  try {
    await (window.electronAPI?.storage as any)?.saveUserSetting(
      'default_cli_project_id',
      defaultCliProject.value
    )
  } catch (error) {
...
<template>
  <div class="space-y-12 animate-in fade-in slide-in-from-bottom-2 duration-500">
    <div class="space-y-8">
          ...
          <div class="mt-10 p-6 bg-gray-950 rounded-2xl border border-white/5 relative group">
            ...
            <div class="font-mono text-[11px] space-y-2">
              ...
              <div class="text-white">
                <span class="text-primary-400">$</span> andb compare dev prod --type tables
              </div>
      ...
      >
        <div
          class="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10"
        >
          <div class="flex items-start gap-4 flex-1">
            ...
            <div>
              ...
              <p
                class="text-xs text-gray-500 dark:text-gray-400 font-medium leading-relaxed max-w-xl"
              >
                Select the target project when executing global command line tools like
                <code
                  class="bg-gray-100 dark:bg-gray-700 px-1 py-0.5 rounded text-primary-500 dark:text-primary-400"
                  >andb compare</code
                >
                without explicitly specifying a project context.
              </p>
            </div>
          </div>
      ...
      <div
        v-if="isCliInstalled"
        class="mt-8 space-y-6 pt-6 border-t border-gray-100 dark:border-gray-800"
      >
        <div class="text-sm text-gray-600 dark:text-gray-300">
          To run
          <code
            class="bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded text-indigo-600 dark:text-indigo-400"
            >andb</code
          >
          from your terminal without requiring administrator privileges, copy and paste one of the
          options below into your terminal.
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            <h3 class="text-sm font-bold text-gray-900 dark:text-white mb-2">
              {{ $t('settings.terminal.option1') }}
            </h3>
            <p class="text-[10px] text-gray-500 dark:text-gray-400 mb-3">
              {{ $t('settings.terminal.option1Desc') }}
            </p>
            <CopyableSnippet
          ...
          >
            <h3 class="text-sm font-bold text-gray-900 dark:text-white mb-2">
              {{ $t('settings.terminal.option2') }}
            </h3>
            <p class="text-[10px] text-gray-500 dark:text-gray-400 mb-3">
              {{ $t('settings.terminal.option2Desc') }}
            </p>
            <CopyableSnippet
              :content="`ln -sf &quot;${internalBinaryPath}&quot; ~/.local/bin/andb`"
              language="bash"
            />
          </div>
        </div>
      ...
      >
          ...
          <div class="flex items-center gap-4">
            ...
            <div>
              <h2 class="text-lg font-black text-gray-900 dark:text-white uppercase tracking-tight">
                {{ $t('settings.terminal.mcpTitle') }}
              </h2>
              <p class="text-xs text-gray-500 dark:text-gray-400 font-medium">
                {{ $t('settings.terminal.mcpDesc') }}
              </p>
            </div>
          </div>
        ...
        <div class="p-8 space-y-6">
          <div
            ...
            v-html="$t('settings.terminal.mcpInfo')"
          ></div>
          ...
          >
            ...
            >
              <h4 class="font-black text-[10px] text-gray-400 uppercase tracking-widest mb-2">
                {{ $t('settings.terminal.cursorConfig') }}
              </h4>
              <code
                class="text-xs text-gray-600 dark:text-gray-400 break-all select-all block bg-white dark:bg-gray-800 p-2 rounded border border-gray-100 dark:border-gray-700"
                >~/Library/Application
            ...
            >
              <h4 class="font-black text-[10px] text-gray-400 uppercase tracking-widest mb-2">
                {{ $t('settings.terminal.windsurfConfig') }}
              </h4>
              <code
                class="text-xs text-gray-600 dark:text-gray-400 break-all select-all block bg-white dark:bg-gray-800 p-2 rounded border border-gray-100 dark:border-gray-700"
