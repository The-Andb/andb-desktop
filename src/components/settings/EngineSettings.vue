  ...
  <div class="animate-in fade-in slide-in-from-bottom-2 duration-500">
    <div class="space-y-12">
        ...
        <div class="relative z-10">
          <div class="flex items-center gap-4 mb-3">
            ...
            <div class="flex flex-col">
              ...
              >
                {{ $t('settings.engine.domainNormalization.title') }}
              </h3>
              <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{{
                $t('settings.engine.domainNormalization.subtitle') || 'Environment Variable Mapping'
              }}</span>
            </div>
          </div>
          <p
            class="text-[11px] text-gray-400 dark:text-gray-500 mb-10 max-w-xl leading-relaxed font-medium"
            v-html="$t('settings.engine.domainNormalization.desc')"
          ></p>

          <div class="space-y-12">
            <TransitionGroup name="list">
              <div
                v-for="(rep, index) in settingsStore.settings.envReplacements || []"
                :key="index"
              ...
              >
                ...
                <button
                  @click="removeEnvReplacementGlobal(index)"
                ...
                <div class="space-y-4 max-w-md">
                  <label
                    class="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.25em]"
                ...
                <div class="pt-10 border-t border-gray-100 dark:border-gray-800/40">
                  <label
                    class="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.25em] mb-8"
                    >Environment Resolution</label
                  >
                  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    ...
                    >
                      <div class="flex items-center gap-2.5">
                        <span
                          class="w-1.5 h-1.5 rounded-full bg-gray-200 dark:bg-gray-800 group-focus-within/field:bg-orange-500 transition-colors"
                        ></span>
                        <span
      ...
      <div class="pt-12 border-t border-gray-100 dark:border-gray-800">
        ...
        >
          {{ $t('settings.engine.globalFilters.title') || 'Global Schema Filters' }}
        </h3>
        <p class="text-[11px] text-gray-400 dark:text-gray-500 mb-8 max-w-xl font-medium">
          {{
            $t('settings.engine.globalFilters.desc') ||
            'Patterns defined here will be excluded from all comparisons across all projects.'
          }}
        </p>
        ...
        <div class="space-y-6">
          <div class="flex items-center gap-3">
            <input
              v-model="newExcludeTagGlobal"
              type="text"
              @keyup.enter="addExcludeTagGlobal"
          ...
          <div class="flex flex-wrap gap-2">
            <div
              v-for="(tag, index) in settingsStore.settings.excludeTags || []"
              :key="index"
            ...
            >
              <span class="text-[10px] font-black text-gray-700 dark:text-gray-300 font-mono">{{
                tag
              }}</span>
              <button
                @click="removeExcludeTagGlobal(index)"
