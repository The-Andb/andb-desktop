...
const buttonStyles = computed<
  { id: 'full' | 'minimal' | 'icons'; label: string; icon: any; desc: string }[]
>(() => [
  {
    id: 'full',
    label: t('settings.interface.buttons.premium'),
    icon: markRaw(Zap),
    desc: t('settings.interface.buttons.premiumDesc')
  },
  {
    id: 'minimal',
    label: t('settings.interface.buttons.minimal'),
    icon: markRaw(MousePointer2),
    desc: t('settings.interface.buttons.minimalDesc')
  },
  {
    id: 'icons',
    label: t('settings.interface.buttons.iconOnly'),
    icon: markRaw(Layers),
    desc: t('settings.interface.buttons.iconOnlyDesc')
  }
])
</script>
  ...
  <div class="space-y-12 animate-in fade-in slide-in-from-bottom-2 duration-500">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <div
                  class="h-1.5 w-full flex gap-0.5 p-0.5 border-b opacity-20"
                ...
                <div class="flex-1 flex overflow-hidden">
                  <div
                    class="w-1/4 h-full border-r opacity-20"
                    :style="{ backgroundColor: t.preview.sidebar, borderColor: 'currentColor' }"
                  ></div>
                  <div class="flex-1 p-2 space-y-1">
                    <div
                      class="h-1 w-2/3 rounded-full opacity-20"
                      :class="t.dark ? 'bg-gray-600' : 'bg-gray-300'"
                    ></div>
                    <div
                      class="h-1.5 w-full rounded border border-dashed opacity-10"
                      :class="t.dark ? 'border-gray-500' : 'border-gray-400'"
                    ></div>
                    <div class="grid grid-cols-3 gap-1 mt-2">
                      <div
                        class="h-3 rounded-sm opacity-20"
                        :class="t.dark ? 'bg-gray-800' : 'bg-gray-100'"
                      ></div>
                      <div
                        class="h-3 rounded-sm opacity-20"
                        :class="t.dark ? 'bg-gray-800' : 'bg-gray-100'"
                      ></div>
                      <div
                        class="h-3 rounded-sm border-l-2 border-primary-500"
                        :style="{
                          backgroundColor: t.id === 'light' ? '#f0fdf4' : t.preview.sidebar + '80'
                        }"
                      ></div>
                    </div>
                  </div>
            ...
            <div
              class="flex flex-col items-center gap-2 px-1 text-center min-h-[40px] justify-start"
            >
              <span
                class="text-[10px] font-black uppercase tracking-[0.15em] transition-colors leading-relaxed"
                :class="
                  settings.theme === t.id
              ...
              <div
                ...
                :class="
                  settings.theme === t.id ? 'bg-primary-500 scale-100' : 'bg-transparent scale-0'
                "
              ></div>
            </div>
          </div>
      ...
      <div class="md:col-span-2 space-y-2 pt-2">
        <label class="block text-xs font-black text-gray-400 uppercase tracking-[0.2em] ml-1">{{
          $t('settings.interface.timezone.label')
        }}</label>
        <div class="relative group">
          <select
      ...
      <div class="md:col-span-2 space-y-4 pt-4 animate-in fade-in slide-in-from-bottom-2">
        <div class="flex items-center justify-between">
          <label class="block text-xs font-black text-gray-400 uppercase tracking-[0.2em] ml-1">{{
            $t('settings.interface.navigation.label')
          }}</label>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button
            ...
            :class="
              appStore.navStyle === 'vertical-list'
          ...
          >
            ...
            <div class="text-left">
              ...
              >
                {{ $t('settings.interface.navigation.vertical') }}
              </div>
              <div class="text-[10px] text-gray-400 uppercase tracking-tighter">
                {{ $t('settings.interface.navigation.verticalDesc') }}
              </div>
            </div>
          </button>
          ...
          >
            ...
            <div class="text-left">
              <div
                class="text-xs font-black text-gray-900 dark:text-white uppercase leading-none mb-1"
              >
                {{ $t('settings.interface.navigation.horizontal') }}
              </div>
              <div class="text-[10px] text-gray-400 uppercase tracking-tighter">
                {{ $t('settings.interface.navigation.horizontalDesc') }}
              </div>
            </div>
          </button>
        </div>
        ...
        >
          ...
          <div class="grid grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            >
              ...
              />
              <span
                class="text-xs font-black text-gray-700 dark:text-gray-300 truncate tracking-tight"
                >{{ item.name }}</span
              >
            </label>
          </div>
    ...
    <div class="pt-8 border-t border-gray-100 dark:border-gray-800">
      <div class="mb-8">
        <h3
          class="text-xs font-black text-gray-600 dark:text-gray-300 uppercase tracking-widest mb-1"
        >
          {{ $t('settings.interface.typography.title') }}
        </h3>
        <p class="text-[10px] text-gray-400 uppercase tracking-tighter">
          {{ $t('settings.interface.typography.subtitle') }}
        </p>
      </div>
      ...
      <div class="mb-6 grid grid-cols-4 gap-4 p-1 bg-gray-100 dark:bg-gray-800 rounded-xl">
        <button
          ...
          :class="
            appStore.fontSizeProfile === profile
        ...
        >
          ...
          <span class="hidden sm:inline">{{
            $t('settings.interface.typography.profiles.' + profile)
          }}</span>
        </button>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
        <div class="space-y-2">
          <label class="block text-xs font-bold text-gray-500 uppercase tracking-tight">{{
            $t('settings.interface.typography.mainFont')
          }}</label>
          <select
            v-model="appStore.fontFamilies.general"
          ...
          >
            <option value="'Inter', sans-serif">
              {{ $t('settings.interface.typography.fonts.inter') }}
            </option>
            <option value="'Roboto', sans-serif">
              {{ $t('settings.interface.typography.fonts.roboto') }}
            </option>
            <option value="'Outfit', sans-serif">
              {{ $t('settings.interface.typography.fonts.outfit') }}
            </option>
            <option
              value="system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif"
            >
              {{ $t('settings.interface.typography.fonts.systemNative') }}
            </option>
          </select>
        </div>
        <div>
          ...
          <select
            v-model="appStore.fontFamilies.code"
          ...
          >
            <option value="'JetBrains Mono', monospace">
              {{ $t('settings.interface.typography.fonts.jetbrains') }}
            </option>
            <option value="'Monaco', Consolas, monospace">Monaco</option>
            <option value="'Cascadia Code', monospace">Cascadia Code</option>
            <option value="'Fira Code', monospace">
              {{ $t('settings.interface.typography.fonts.fira') }}
            </option>
            <option value="'Source Code Pro', monospace">
              {{ $t('settings.interface.typography.fonts.sourceCode') }}
            </option>
            <option
              value="ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace"
            >
              {{ $t('settings.interface.typography.fonts.systemNativeMono') }}
            </option>
          </select>
        </div>

        <div
          v-if="appStore.fontSizeProfile === 'custom'"
        ...
        >
          <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4">{{
            $t('settings.interface.typography.granularMatrix')
          }}</label>
          <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            <div
              v-for="(_, key) in appStore.fontSizes"
              :key="key"
            ...
            >
              <label
                class="block text-xs font-bold text-gray-400 uppercase tracking-tight mb-2 truncate"
                >{{ $t('settings.interface.typography.sizeLabels.' + key) }}</label
              >
              <select
                v-model.number="appStore.fontSizes[key]"
    ...
    <div
      class="pt-8 border-t border-gray-100 dark:border-gray-800 animate-in fade-in slide-in-from-bottom-2"
    >
      <div class="mb-8">
        <h3
          class="text-sm font-black text-gray-600 dark:text-gray-300 uppercase tracking-widest mb-1"
        >
          {{ $t('settings.interface.buttons.title') }}
        </h3>
        <p class="text-xs text-gray-400 uppercase tracking-tighter">
          {{ $t('settings.interface.buttons.subtitle') }}
        </p>
      </div>
      ...
      <div class="flex flex-col lg:flex-row gap-8">
        <div class="lg:w-1/3 space-y-3">
          ...
          >
            <div class="flex items-center gap-4">
              <div
                ...
                :class="
                  appStore.buttonStyle === style.id
                    ? 'bg-primary-500 text-white'
                    : 'bg-gray-100 dark:bg-gray-800 text-gray-400 group-hover:text-primary-500'
                "
              >
            ...
            <div
              v-if="appStore.buttonStyle === style.id"
        ...
        >
          ...
          <div class="flex flex-col items-center gap-10 w-full max-w-sm">
            <div class="w-full flex flex-col items-center">
              <span
                class="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 opacity-60"
              ...
              <button
                ...
                :class="[
                  appStore.buttonStyle === 'full'
                    ? 'px-10 py-3.5 bg-gradient-to-r from-primary-600 to-primary-500 text-white rounded-2xl text-[11px] tracking-[0.2em] shadow-xl shadow-primary-500/30'
                    : '',
                  appStore.buttonStyle === 'minimal'
                    ? 'px-8 py-2.5 bg-primary-500 text-white rounded-xl text-[10px] tracking-widest shadow-lg shadow-primary-500/10'
                    : '',
                  appStore.buttonStyle === 'icons'
                    ? 'w-14 h-14 bg-primary-500 text-white rounded-full shadow-2xl shadow-primary-500/30 hover:scale-110'
                    : ''
                ]"
              >
                <Zap
                  class="w-5 h-5"
                  :class="appStore.buttonStyle !== 'icons' ? 'mr-3' : ''"
                  :fill="appStore.buttonStyle === 'full' ? 'currentColor' : 'none'"
                />
                <span v-if="appStore.buttonStyle !== 'icons'">{{
                  appStore.buttonStyle === 'full'
                    ? $t('settings.interface.preview.initialize')
                    : $t('settings.interface.preview.analyze')
                }}</span>
              </button>
            </div>
            ...
            <div class="flex items-center gap-16">
              <div class="flex flex-col items-center">
                <span
                  class="text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 opacity-60"
                  >{{ $t('settings.interface.preview.secondary') }}</span
                >
                <button
                  class="transition-all duration-300 active:scale-95 hover:scale-105"
                  :class="[
                    appStore.buttonStyle === 'full'
                      ? 'flex items-center gap-3 px-6 py-3 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-gray-700 rounded-2xl text-[11px] font-black shadow-md'
                      : '',
                    appStore.buttonStyle === 'minimal'
                      ? 'flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 rounded-xl text-[10px] font-black'
                      : '',
                    appStore.buttonStyle === 'icons'
                      ? 'w-11 h-11 flex items-center justify-center bg-white dark:bg-gray-800 text-gray-500 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-sm'
                      : ''
                  ]"
                >
                  <HistoryIcon class="w-4 h-4" />
                  <span v-if="appStore.buttonStyle !== 'icons'">{{
                    $t('settings.interface.preview.checkHistory')
                  }}</span>
                </button>
              </div>
              <div class="flex flex-col items-center">
                ...
                >
                <button
                  class="transition-all duration-300 active:scale-95 group/u"
                  :class="[
                    appStore.buttonStyle === 'full'
                      ? 'flex items-center justify-center w-12 h-12 bg-indigo-50 dark:bg-indigo-950/30 text-indigo-500 border border-indigo-100 dark:border-indigo-900/50 rounded-2xl shadow-inner hover:bg-indigo-100 transition-colors'
                      : '',
                    appStore.buttonStyle === 'minimal'
                      ? 'flex items-center justify-center w-10 h-10 bg-gray-50 dark:bg-gray-800/80 text-gray-400 rounded-xl'
                      : '',
                    appStore.buttonStyle === 'icons'
                      ? 'text-gray-400 hover:text-primary-500 hover:scale-125 transition-transform'
                      : ''
                  ]"
                >
                  <SettingsIcon
                    class="w-4.5 h-4.5 group-hover/u:rotate-90 transition-transform duration-500"
                  />
                </button>
              </div>
            </div>
