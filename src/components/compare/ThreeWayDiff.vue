...
const diffLines = computed(() => {
  ...
  for (let i = 0; i < maxLines; i++) {
    ...
    if (s !== t) {
      if (s && t)
        status = 'conflict' // Both exist but differ (Modify)
      else if (s && !t)
        status = 'source-only' // Add (Safe)
  ...
  <div class="flex-1 flex flex-col overflow-hidden bg-background-dark">
    ...
    <div class="flex flex-1 overflow-hidden border-b border-slate-800">
      ...
      <div class="flex-1 flex flex-col min-w-0 border-r border-slate-800">
        <div
          class="h-10 px-4 flex items-center justify-between bg-slate-900 border-b border-slate-800 shrink-0"
        >
        ...
        <div class="flex-1 overflow-auto bg-[#0d1117] p-2 font-mono text-sm leading-6">
          ...
          >
            <span class="w-8 text-right pr-3 text-slate-600 select-none text-[10px] pt-1">{{
              line.index
            }}</span>
            <span class="whitespace-pre flex-1 pl-2" :class="getLineClass(line.status, 'source')">{{
              line.source || ' '
            }}</span>
          </div>
        </div>
      ...
      <div class="flex-1 flex flex-col min-w-0">
        ...
        <div class="flex-1 overflow-auto bg-[#0d1117] p-2 font-mono text-sm leading-6">
          ...
          >
            ...
            <span class="whitespace-pre flex-1 pl-2" :class="getLineClass(line.status, 'target')">{{
              line.target || ' '
            }}</span>
          </div>
        </div>
    ...
    <div class="h-1/3 min-h-[200px] flex flex-col bg-panel-dark">
      ...
      >
        <div class="flex items-center gap-3">
          <span class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{{
            $t('compare.mergedPreview')
          }}</span>
          <span
            class="text-[10px] px-1.5 py-0.5 bg-amber-500/10 text-amber-500 border border-amber-500/20 rounded"
            >{{ $t('compare.reviewRequired') }}</span
          >
        </div>
      </div>
      <div class="flex-1 p-4 bg-[#0d1117] font-mono text-sm text-slate-300 overflow-auto">
        ...
        <template v-for="line in diffLines" :key="`r-${line.index}`">
          ...
          <div v-else-if="line.status === 'source-only'" class="text-emerald-400">
            {{ line.source }}
          </div>
