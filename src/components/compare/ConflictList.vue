...
<template>
  <div class="w-72 border-r border-slate-800 bg-panel-dark flex flex-col shrink-0">
    <div class="p-4 border-b border-slate-800 flex items-center justify-between">
      ...
      <span class="bg-slate-800 text-slate-400 text-[10px] px-2 py-0.5 rounded-full"
        >{{ items.length }} Objects</span
      >
    </div>
    ...
    <div class="flex-1 overflow-y-auto">
      ...
      >
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 min-w-0">
            ...
            >
              {{
                item.status === 'resolved'
                  ? 'Resolved'
                  : `${item.conflictLineCount} conflicting lines`
              }}
            </p>
          </div>
          ...
          <span v-else class="material-symbols-outlined text-emerald-500 text-sm"> check </span>
        </div>
      </div>
