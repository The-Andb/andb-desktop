  ...
  >
    ...
    <template v-if="viewType === 'split'">
      <div class="flex-1 flex flex-col min-h-0 min-w-0 max-w-full overflow-hidden relative">
        <!-- Headers -->
        <div
          class="flex shrink-0 h-10 border-b border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-900 relative"
        >
          <!-- Source Header -->
          <div
            :style="{ width: leftPaneWidth + '%' }"
            class="px-4 py-2 flex justify-between items-center shrink-0"
          >
            <span
              class="font-bold text-primary-600 dark:text-primary-400 opacity-80 uppercase tracking-widest text-[10px]"
              >{{ $t('compare.diffView.source', { label: sourceLabel }) }}</span
            >
            <span
              v-if="isEmptySource"
              class="text-[10px] bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-400 px-1.5 py-0.5 rounded border border-red-200 dark:border-red-800/50 font-bold uppercase"
              >{{ $t('compare.diffView.deleted') }}</span
            >
          </div>
          ...
          <div class="flex-1 px-4 py-2 flex justify-between items-center">
            <span
              class="font-bold text-emerald-600 dark:text-emerald-400 opacity-80 uppercase tracking-widest text-[10px]"
              >{{ $t('compare.diffView.target', { label: targetLabel }) }}</span
            >
            <div class="flex items-center gap-3">
              ...
              <div class="relative" ref="settingsRef">
                ...
                <div
                  v-if="showSettings"
                  class="absolute right-0 top-full mt-2 w-64 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 p-4 z-50 text-xs text-gray-600 dark:text-gray-300 pointer-events-auto"
                >
                  <h3
                    class="font-bold text-gray-900 dark:text-white mb-3 uppercase tracking-widest text-[10px]"
                  >
                    {{ $t('compare.diffView.settings') }}
                  </h3>
                  <div class="space-y-4 text-left">
                    <!-- Whitespace -->
                    <div>
                      <h4 class="font-bold text-gray-500 uppercase text-[9px] mb-2">
                        {{ $t('compare.diffView.whitespace') }}
                      </h4>
                      <label class="flex items-start cursor-pointer group">
                        <div class="relative flex items-center mt-0.5">
                          ...
                          <div
                            ...
                            :class="{ 'bg-primary-500 border-primary-500': hideWhitespace }"
                          >
                        ...
                        <div class="ml-2">
                          <div class="text-gray-900 dark:text-white font-medium">
                            {{ $t('compare.diffView.hideWhitespace') }}
                          </div>
                          <div class="text-[10px] text-gray-400 mt-0.5 leading-tight">
                            {{ $t('compare.diffView.hideWhitespaceDesc') }}
                          </div>
                        </div>
                      </label>
                      <label class="flex items-start cursor-pointer group mt-3">
                        <div class="relative flex items-center mt-0.5">
                          ...
                          <div
                            class="w-4 h-4 border rounded border-gray-300 dark:border-gray-600 group-hover:border-primary-500 transition-colors flex items-center justify-center font-bold"
                            :class="{ 'bg-primary-500 border-primary-500': internalIgnoreCase }"
                          >
                        ...
                        <div class="ml-2">
                          <div class="text-gray-900 dark:text-white font-medium">
                            {{ $t('compare.diffView.ignoreCase') }}
                          </div>
                          <div class="text-[10px] text-gray-400 mt-0.5 leading-tight">
                            {{ $t('compare.diffView.ignoreCaseDesc') }}
                          </div>
                        </div>
                      </label>
                      <label class="flex items-start cursor-pointer group mt-3">
                    ...
                    <div>
                      ...
                      <label class="flex items-start cursor-pointer group">
                        <div class="relative flex items-center mt-0.5">
                          ...
                          <div
                            class="w-4 h-4 border rounded border-gray-300 dark:border-gray-600 group-hover:border-primary-500 transition-colors flex items-center justify-center font-bold"
                            :class="{ 'bg-primary-500 border-primary-500': wrapLines }"
                          >
                    ...
                    <div>
                      <h4 class="font-bold text-gray-500 uppercase text-[9px] mb-2">
                        {{ $t('compare.diffView.diffDisplay') }}
                      </h4>
                      <div class="space-y-2">
                        <label
                          v-for="mode in ['Unified', 'Split']"
                          :key="mode"
                        ...
                        >
                          ...
                          />
                          <div
                            class="w-4 h-4 rounded-full border border-gray-300 dark:border-gray-600 group-hover:border-primary-500 transition-colors flex items-center justify-center p-1"
                            :class="{ 'border-primary-500': viewType === mode.toLowerCase() }"
                          >
                            <div
                              v-show="viewType === mode.toLowerCase()"
                              class="w-2 h-2 rounded-full bg-primary-500"
                            ></div>
                          </div>
                          <span class="ml-2 text-gray-900 dark:text-white font-medium">{{
                            $t('compare.diffView.' + mode.toLowerCase() + 'Mode')
                          }}</span>
                        </label>
                      </div>
                    </div>
        ...
        <div class="flex-1 flex overflow-hidden relative bg-gray-50 dark:bg-gray-950">
          <div
            v-if="isEmptySource && isEmptyTarget"
            class="placeholder-empty flex items-center justify-center w-full h-full text-gray-600 italic"
          >
              ...
              <template v-for="(chunk, cIdx) in alignedChunks" :key="'sp-chk-' + chunk.id">
                ...
                <template v-if="chunk.type === 'visible'">
                  ...
                  >
                    ...
                    >
                      <div
                        class="line-number w-12 shrink-0 text-right px-2 py-0.5 text-gray-400 dark:text-gray-600 select-none border-r border-gray-100 dark:border-[#30363d] group-hover:text-gray-600 dark:group-hover:text-gray-400 bg-gray-100/30 dark:bg-gray-800/20"
                      >
                        {{ row.source.line || '' }}
                      </div>
                      <div
                        class="line-marker w-5 shrink-0 flex items-center justify-center opacity-70 select-none font-bold"
                      >
                    ...
                    <!-- Target Side -->
                    <div class="flex-1 flex line-row group" :class="getLineClass(row.target.type)">
                      <div
                        class="line-number w-12 shrink-0 text-right px-2 py-0.5 text-gray-400 dark:text-gray-600 select-none border-r border-gray-100 dark:border-[#30363d] group-hover:text-gray-600 dark:group-hover:text-gray-400 bg-gray-100/30 dark:bg-gray-800/20"
                      >
                        {{ row.target.line || '' }}
                ...
                <div
                  v-else
                  class="w-full flex items-center justify-center bg-gray-50/30 dark:bg-gray-900/30 border-y py-2 border-gray-100 dark:border-[#30363d]/50 relative group/expandbtn shadow-inner isolate"
                  style="height: 48px"
                >
                  <div
                  ...
                  >
                    <div
                      class="absolute top-0 bottom-0 w-[1px] bg-gray-200 dark:bg-gray-800/50 pointer-events-none z-0"
                      :style="{ left: `calc(${leftPaneWidth}% - 1px)` }"
                    ></div>
                  </div>

                  <div
                    class="flex items-center isolate z-10 bg-white dark:bg-gray-800 rounded-full shadow-sm ring-1 ring-black/5 dark:ring-white/10 p-0.5 pointer-events-auto transition-transform hover:scale-105"
                  >
                    <button
                      class="w-8 h-6 flex items-center justify-center rounded-l-full hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-400 hover:text-primary-500 transition-colors"
                      @click.stop="expandFromTop(cIdx)"
                      :disabled="cIdx === 0"
                      :class="{ 'opacity-30 cursor-not-allowed': cIdx === 0 }"
                    >
                      <ChevronDown class="w-4 h-4" />
                    </button>
                    <button
                      class="px-3 h-6 flex items-center justify-center text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors"
                      @click.stop="expandAll(cIdx)"
                    >
                      {{ chunk.rows.length }} Lines
                    </button>
                    <button
                      class="w-8 h-6 flex items-center justify-center rounded-r-full hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-400 hover:text-primary-500 transition-colors"
                      @click.stop="expandFromBottom(cIdx)"
                      :disabled="cIdx >= alignedChunks.length - 1"
                      :class="{ 'opacity-30 cursor-not-allowed': cIdx >= alignedChunks.length - 1 }"
                    >
    ...
    <div v-else class="flex-1 flex flex-col min-h-0 overflow-hidden">
      <div
        class="sticky top-0 z-10 bg-gray-100 dark:bg-gray-900 px-4 py-2 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center shrink-0 h-10"
      >
        <span
          class="font-bold text-primary-600 dark:text-primary-400 opacity-80 uppercase tracking-widest text-[10px]"
          >{{ $t('compare.diffView.unified', { source: sourceLabel, target: targetLabel }) }}</span
        >
      ...
      <div class="flex-1 overflow-auto custom-scrollbar-diff relative ddl-container">
        <template v-for="(chunk, cIdx) in unifiedChunks" :key="'uni-chk-' + chunk.id">
          <template v-if="chunk.type === 'visible'">
            ...
            >
              <div
                class="line-numbers w-24 shrink-0 flex border-r border-gray-100 dark:border-[#30363d] select-none text-[10px]"
              >
                <div
                  class="w-12 text-right px-2 py-0.5 text-gray-400 dark:text-gray-600 border-r border-gray-50 dark:border-gray-800/50"
                >
                  {{ row.sourceLine || '' }}
                </div>
          ...
          <div
            v-else
            class="flex items-center justify-center bg-gray-50/30 dark:bg-gray-900/30 border-y py-2 border-gray-100 dark:border-[#30363d]/50 relative group/expandbtn shadow-inner"
            style="height: 48px"
          >
            <div
              class="flex items-center isolate z-10 bg-white dark:bg-gray-800 rounded-full shadow-sm ring-1 ring-black/5 dark:ring-white/10 p-0.5 pointer-events-auto transition-transform hover:scale-105"
            >
              ...
              <button
                class="w-8 h-6 flex items-center justify-center rounded-r-full hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-400 hover:text-primary-500 transition-colors"
                @click.stop="expandFromBottom(cIdx)"
                :disabled="cIdx >= unifiedChunks.length - 1"
                :class="{ 'opacity-30 cursor-not-allowed': cIdx >= unifiedChunks.length - 1 }"
                title="Expand Up"
              >
                <ChevronUp class="w-4 h-4" />
              </button>
...
const isEmptyTarget = computed(
  () => !props.targetDdl || props.status === 'missing_in_target' || props.status === 'missing'
)
        ...
        if (seg.rows.length <= keepTop + keepBottom) {
          ...
          if (keepTop > 0)
            chunks.push({
              id: `c${chunkIdCounter++}`,
              type: 'visible',
              rows: seg.rows.slice(0, keepTop)
            })
          const collapsedRows = seg.rows.slice(keepTop, seg.rows.length - keepBottom)
          if (collapsedRows.length > 0)
            chunks.push({ id: `c${chunkIdCounter++}`, type: 'collapsed', rows: collapsedRows })
          if (keepBottom > 0)
            chunks.push({
              id: `c${chunkIdCounter++}`,
              type: 'visible',
              rows: seg.rows.slice(seg.rows.length - keepBottom)
            })
        }
      }
    }
...
const expandFromTop = (index: number) => {
  ...
  if (alignedChunks.value[index - 1] && alignedChunks.value[index - 1].type === 'visible') {
    ...
    alignedChunks.value.splice(index, 0, {
      id: `c${chunkIdCounter++}`,
      type: 'visible',
      rows: revealed
    })
...
const expandFromBottom = (index: number) => {
  ...
  if (alignedChunks.value[index + 1] && alignedChunks.value[index + 1].type === 'visible') {
    ...
    alignedChunks.value.splice(index + 1, 0, {
      id: `c${chunkIdCounter++}`,
      type: 'visible',
      rows: revealed
    })
    ...
    if (i > 0 && j > 0 && compare(sourceLines[i - 1], targetLines[j - 1])) {
      ...
      result.unshift({
        source: {
          ...
          highlighted: highlightedSourceLines.value[i - 1],
          type: 'equal'
        },
        target: {
          line: j,
          content: targetLines[j - 1],
          highlighted: highlightedTargetLines.value[j - 1],
          type: 'equal'
        }
      })
      i--
...
const handleClickOutside = (event: MouseEvent) => {
  ...
  const isOutsideUnified =
    settingsRefUnified.value && !settingsRefUnified.value.contains(event.target as Node)
