<template>
  ...
  >
    ...
    >
      ...
      >
        <span
          class="text-[9px] font-black uppercase tracking-widest text-gray-400 dark:text-gray-500 shrink-0"
          >SRC</span
        >
        <Flame v-if="appStore.compareStack.source" class="w-3.5 h-3.5 text-orange-500 shrink-0" />
        <span
          v-else
          class="w-3.5 h-3.5 rounded-full border border-dashed border-gray-300 dark:border-gray-500 shrink-0"
        ></span>
        <span
          class="text-xs font-mono truncate font-medium tracking-tight flex-1 ml-0.5"
          :class="
            appStore.compareStack.source
        ...
        <button
          ...
          @click="appStore.compareStack.source = null"
          class="w-5 h-5 rounded-full flex items-center justify-center text-gray-400 hover:bg-orange-100 dark:hover:bg-orange-900/50 hover:text-orange-600 transition-colors shrink-0"
        >
          <X class="w-3 h-3" />
        </button>
      </div>
      ...
      >
        ...
        <button
          ...
          @click="appStore.compareStack.target = null"
          class="w-5 h-5 rounded-full flex items-center justify-center text-gray-400 hover:bg-blue-100 dark:hover:bg-blue-900/50 hover:text-blue-600 transition-colors shrink-0"
        >
          <X class="w-3 h-3" />
        </button>
      </div>
      ...
      <div class="flex items-center gap-1.5 ml-1 shrink-0">
        ...
        <button
          @click="appStore.clearCompareStack()"
          class="w-8 h-8 rounded-full flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all shrink-0"
          title="Clear Stack"
        >
