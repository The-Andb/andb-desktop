<template>
  <div
    class="flex flex-col h-full bg-white dark:bg-gray-900 overflow-hidden select-none"
    @mouseup="stopResize"
  ...
  >
    ...
    >
      <!-- Type Filters -->
      <div
        class="flex items-center gap-1.5 pr-3 mr-3 border-r border-gray-100 dark:border-gray-700"
      >
        <button
    ...
    <!-- Tree Content -->
    <div
      class="flex-1 overflow-auto custom-scrollbar p-4 relative min-h-0"
      ...
      <div v-else class="space-y-6">
        <div v-for="category in categories" :key="category.type" class="space-y-1">
          ...
          >
            ...
            <span
              class="font-black text-[11px] uppercase tracking-[0.2em] text-gray-900 dark:text-gray-100"
              >{{ category.type }}</span
            >
            <span
            ...
            >
              ...
              >
                <div
                  ...
                  :class="{
                    'bg-orange-50 dark:bg-orange-900/20 ring-1 ring-orange-200 dark:ring-orange-800/50':
                      appStore.compareStack?.source?.name === item.name && hasInSource(item)
                  }"
                >
                  <div class="flex items-center truncate py-1">
                    <template v-if="hasInSource(item)">
                      <component
                        :is="getCategoryIcon(category.type)"
                        class="w-3.5 h-3.5 mr-2 opacity-50 shrink-0"
                      />
                      <span class="truncate font-mono" :class="getSourceClass(item)">{{
                        item.name
                      }}</span>
                    </template>
                    <template v-else>
                      <span class="text-gray-300 dark:text-gray-600 italic text-[10px] pl-6">{{
                        $t('compare.treeViewData.missingSource')
                      }}</span>
                    </template>
                  </div>
                  ...
                  <button
                    ...
                    :class="[
                      appStore.compareStack?.source?.name === item.name
                        ? 'opacity-100 bg-orange-500 text-white dark:bg-orange-600 shadow-sm'
                        : 'opacity-0 group-hover/left:opacity-100 text-gray-400 hover:text-orange-500 hover:bg-orange-100 dark:hover:bg-orange-900/30'
                    ]"
                    ...
                    <Workflow
                      ...
                      :class="
                        appStore.compareStack?.source?.name === item.name
                          ...
                          : 'text-orange-400 group-hover/btn:text-orange-500'
                      "
                    />
                    <span
                      v-if="appStore.compareStack?.source?.name === item.name"
                      class="text-[9px] font-black tracking-widest uppercase pr-1"
                      >SRC</span
                    >
                  </button>
                </div>
              ...
              <div class="shrink-0 w-12 flex justify-center z-10 relative">
                ...
                >
                  ...
                  <component
                    ...
                    :class="[
                      ...
                      canMigrate(item) ? 'group-hover/icon:hidden' : ''
                    ]"
                  />
                </div>
                ...
                <div
                  ...
                  class="absolute left-full top-1/2 w-4 h-px bg-gray-200 dark:bg-gray-700/50 -ml-1"
                ></div>
              </div>
                ...
                <div
                  ...
                  :class="{
                    'bg-blue-50 dark:bg-blue-900/20 ring-1 ring-blue-200 dark:ring-blue-800/50':
                      appStore.compareStack?.target?.name === item.name && hasInTarget(item)
                  }"
                >
                  ...
                  <button
                    ...
                    :class="[
                      appStore.compareStack?.target?.name === item.name
                        ? 'opacity-100 bg-blue-500 text-white dark:bg-blue-600 shadow-sm'
                        : 'opacity-0 group-hover/right:opacity-100 text-gray-400 hover:text-blue-500 hover:bg-blue-100 dark:hover:bg-blue-900/30'
                    ]"
                    title="Set as Target Base"
                  >
                    <span
                      v-if="appStore.compareStack?.target?.name === item.name"
                    ...
                    <Workflow
                      ...
                      :class="
                        appStore.compareStack?.target?.name === item.name
                          ? 'text-white fill-white/20'
                          : 'text-blue-400 group-hover/btn:text-blue-500'
                      "
                    />
                  </button>
                  ...
                  <div class="flex items-center truncate py-1">
                    <template v-if="hasInTarget(item)">
                      <component
                        ...
                        class="w-3.5 h-3.5 mr-2 opacity-50 shrink-0"
                      />
                      <span class="truncate font-mono" :class="getTargetClass(item)">{{
                        item.name
                      }}</span>
                    </template>
                    <template v-else>
                      <span class="text-gray-300 dark:text-gray-600 italic text-[10px] pl-4">{{
                        $t('compare.treeViewData.missingTarget')
                      }}</span>
                    </template>
                  </div>
                </div>
...
const props = defineProps<{
  ...
  treeExpandCmd?: { action: 'expand' | 'collapse' | ''; ts: number } | null
}>()
  ...
  cmd => {
    ...
    if (cmd.action === 'expand') {
      ...
      ;['tables', 'views', 'procedures', 'functions', 'triggers'].forEach(c =>
        collapsedCategories.value.add(c)
      )
    }
  }
)
...
const filterOptions = computed(() => [
  {
    id: 'all',
    label: t('compare.treeViewData.filter.all'),
    activeClass: 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200',
    icon: Database
  },
  {
    id: 'new',
    label: t('compare.treeViewData.filter.new'),
    activeClass: 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400',
    icon: ArrowRight
  },
  {
    id: 'modified',
    label: t('compare.treeViewData.filter.modified'),
    activeClass: 'bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400',
    icon: AlertCircle
  },
  {
    id: 'identical',
    label: t('compare.treeViewData.filter.identical'),
    activeClass: 'bg-teal-50 dark:bg-teal-900/30 text-teal-700 dark:text-teal-400',
    icon: CheckCircle2
  },
  {
    id: 'deprecated',
    label: t('compare.treeViewData.filter.deprecated'),
    activeClass: 'bg-rose-50 dark:bg-rose-900/30 text-rose-700 dark:text-rose-400',
    icon: XCircle
  },
  {
    id: 'excluded',
    label: t('compare.treeViewData.filter.excluded', 'Excluded'),
    activeClass: 'bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300',
    icon: Ban
  }
])
    ...
    .map(type => {
      ...
      if (currentFilter.value !== 'all') {
        items = items.filter(item => {
          ...
          if (currentFilter.value === 'modified')
            return s === 'modified' || s === 'different' || s === 'updated'
          if (currentFilter.value === 'identical')
            return s === 'equal' || s === 'same' || s === 'identical'
          if (currentFilter.value === 'deprecated')
...
const getSourceClass = (item: any) => {
  if (item.status === 'modified' || item.status === 'different')
    return 'text-amber-600 dark:text-amber-400 font-bold'
  if (item.status === 'new' || item.status === 'missing_in_target')
