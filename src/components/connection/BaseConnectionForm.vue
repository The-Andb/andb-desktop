<template>
  <div class="space-y-8">
    ...
    <div class="space-y-6">
      <div
        class="pb-2 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between"
      >
        <h3
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div v-if="!hideType" class="space-y-2">
          ...
          <div class="relative group">
            ...
            <ChevronDown
              v-if="!readOnlyFields.includes('type')"
        ...
        <div class="space-y-2">
          <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">
            {{
              ['dump', 'sqlite'].includes(modelValue.type)
                ? modelValue.type === 'dump'
                  ? $t('connections.dumpPath')
                  : 'SQLite DB Path'
                : $t('connections.host')
            }}
            *
          ...
          <div class="relative">
            <input
              ...
              :placeholder="
                ['dump', 'sqlite'].includes(modelValue.type)
                  ? modelValue.type === 'dump'
                    ? $t('connections.dumpPathPlaceholder')
                    : 'Select or enter path to .sqlite file'
                  : $t('connections.hostPlaceholder')
              "
            ...
            <button
              v-if="
                ['dump', 'sqlite'].includes(modelValue.type) && !readOnlyFields.includes('host')
              "
            ...
            <Lock
              v-if="readOnlyFields.includes('host')"
              class="absolute right-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 pointer-events-none"
            />
          </div>
        </div>
        ...
        <div v-if="!['dump', 'sqlite'].includes(modelValue.type)" class="space-y-2">
          ...
          <div class="relative">
            ...
            />
            <Lock
              v-if="readOnlyFields.includes('port')"
              class="absolute right-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 pointer-events-none"
            />
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-2">
          ...
          <div class="relative">
            ...
            />
            <Lock
              v-if="readOnlyFields.includes('username')"
              class="absolute right-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 pointer-events-none"
            />
          </div>
        </div>
    ...
    <div v-if="!['dump', 'sqlite'].includes(modelValue.type)" class="space-y-6 pt-4">
      ...
      >
        ...
        <label
          ...
          :class="{ 'pointer-events-none opacity-50': readOnlyFields.includes('ssh.enabled') }"
        >
          <div class="relative">
            <input
              type="checkbox"
      ...
      <div
        v-if="modelValue.ssh?.enabled"
        class="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in slide-in-from-top-2 duration-300"
      >
        <!-- SSH Host -->
        <div class="space-y-2">
          <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1"
          ...
          <div class="relative">
            ...
            />
            <Lock
              v-if="readOnlyFields.includes('ssh.host')"
              class="absolute right-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 pointer-events-none"
            />
          </div>
        </div>
        ...
        <div class="space-y-2">
          ...
          <div class="relative">
            ...
            />
            <Lock
              v-if="readOnlyFields.includes('ssh.port')"
              class="absolute right-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 pointer-events-none"
            />
          </div>
        </div>
        ...
        <div class="space-y-2">
          ...
          <div class="relative">
            ...
            />
            <Lock
              v-if="readOnlyFields.includes('ssh.username')"
              class="absolute right-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 pointer-events-none"
            />
          </div>
        </div>
        ...
        <div class="space-y-2 col-span-1 md:col-span-2">
          ...
          <div class="relative">
            ...
            />
            <Lock
              v-if="readOnlyFields.includes('ssh.jumpHost')"
              class="absolute right-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 pointer-events-none"
            />
          </div>
        </div>
...
const sshAuthMethod = ref<'password' | 'key'>(
  props.modelValue.ssh?.privateKeyPath ? 'key' : 'password'
)
