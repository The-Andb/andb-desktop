<template>
  <div class="space-y-6">
      ...
      <div class="space-y-4 pt-1">
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="flex items-center gap-2">
              ...
              >
                <button
                  type="button"
                  v-for="env in ['ALL', 'NONE', 'DEV', 'UAT', 'STAGE', 'PROD', 'LOCAL']"
                  :key="env"
          ...
          <div class="flex items-center justify-end gap-2 ml-auto w-full sm:w-auto">
            ...
            >
              <span class="text-xs text-gray-500 font-bold"
                >{{ selectedTemplates.length }} selected</span
              >
              <Transition name="slide-left" mode="out-in">
                <div
                  v-if="confirmingBulkDelete"
                  class="flex items-center gap-3 px-4 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm animate-in fade-in slide-in-from-right-4 duration-200"
                >
                  <span
                    class="text-[11px] font-black text-gray-900 dark:text-white uppercase tracking-widest"
                    >Delete {{ selectedTemplates.length }}?</span
                  >
                  <button
                    @click="confirmingBulkDelete = false"
                    class="text-[11px] font-black uppercase text-gray-400 hover:text-gray-600 px-2"
                  >
                    No
                  </button>
                  <button
            ...
            <div class="flex items-center gap-2">
              <Transition name="slide-left" mode="out-in">
                <div
                  v-if="confirmingClean"
                  class="flex items-center gap-3 px-4 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl animate-in fade-in slide-in-from-right-4 duration-200 shadow-sm"
                >
                  <span
                    class="text-[11px] font-black text-gray-900 dark:text-white uppercase tracking-widest"
                    >Clean Empty?</span
                ...
                <button
                  ...
                  title="Clean Empty Connections without Hosts"
                >
        ...
        <div class="card overflow-hidden border border-gray-200 dark:border-gray-800 rounded-2xl">
            ...
            <table class="w-full">
              ...
              >
                <tr>
                  ...
                  <th
                    class="px-4 py-2 text-left text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest"
                  >
                  ...
                  <th
                    class="px-4 py-2 text-right text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest"
                  >
                    {{ $t('common.actions') }}
                  </th>
                </tr>
              ...
              >
                <template v-for="group in connectionGroups" :key="group.id">
                  ...
                  <tr v-if="group.templates.length > 0">
                    <td
                      colspan="5"
                      class="px-6 py-2 bg-gray-50 dark:bg-gray-800/50 text-xs font-black text-gray-500 uppercase tracking-widest border-y border-gray-100 dark:border-gray-800"
                    >
                      <div class="flex items-center gap-2">
                        <Database class="w-3.5 h-3.5" /> {{ group.label }}
                      </div>
                    </td>
                  </tr>
                  <tr
                    ...
                    :class="{
                      'bg-primary-50 dark:bg-primary-900/10': selectedTemplates.includes(
                        template.id
                      )
                    }"
                  >
                    ...
                    <td class="px-4 py-2 whitespace-nowrap">
                      <div class="flex items-center">
                        <div class="flex-shrink-0 h-8 w-8">
                          <div
                            class="h-8 w-8 rounded-lg flex items-center justify-center transition-colors shadow-sm cursor-pointer hover:scale-105"
                            :class="{
                              'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400':
                                template.type === 'mysql',
                              'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400':
                                template.type === 'postgres',
                              'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400':
                                template.type === 'sqlite' || !!(template as any).permissions,
                          ...
                          >
                            <ShieldCheck
                              v-if="!!(template as any).permissions"
                              class="w-3.5 h-3.5"
                            />
                            <span
                              v-else-if="template.type === 'postgres'"
                              class="text-[8px] font-black"
                              >PG</span
                            >
                            <span
                              v-else-if="template.type === 'sqlite'"
                              class="text-[8px] font-black"
                              >SL</span
                            >
                            <span v-else-if="template.type === 'dump'" class="text-[8px] font-black"
                              >DP</span
                            >
                        ...
                        <div class="ml-3 cursor-pointer" @click="openForm(template)">
                          <div class="flex items-center gap-2">
                            ...
                            <div class="flex items-center gap-1" @click.stop>
                              <span
                                v-for="env in (template.environment || '')
                                  .split(',')
                                  .filter(Boolean)"
                                :key="env"
                          ...
                          <div
                            class="text-[8px] font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-1 mt-0.5"
                          >
                            {{ template.type }}
                            <span
                              v-if="!!(template as any).permissions"
                              class="text-emerald-500 flex items-center gap-1"
                              >· <ShieldCheck class="w-2 h-2" /> restricted</span
                            >
                          </div>
                        </div>
                      </div>
                    ...
                    <td class="px-4 py-2 whitespace-nowrap">
                      <div class="flex flex-col max-w-[200px]">
                        <div
                          class="flex items-center gap-2 text-xs font-medium text-gray-700 dark:text-gray-300 overflow-hidden"
                        >
                          <Server class="w-3 h-3 text-gray-400 shrink-0" />
                          <span class="truncate" :title="template.host">{{
                            template.host || '(No Host)'
                          }}</span>
                          <span
                            v-if="template.type !== 'sqlite' && template.port"
                            class="shrink-0 text-[10px] opacity-75"
                            >:{{ template.port }}</span
                          >
                        </div>
                        <div
                          class="flex items-center gap-2 text-[10px] mt-0.5"
                          :class="
                            !!(template as any).permissions
                    ...
                    <td class="px-4 py-2 whitespace-nowrap">
                      <div
                        v-if="template.database"
                        class="flex items-center gap-1.5 text-xs text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded w-fit"
                      >
                    ...
                    <td class="px-4 py-2 whitespace-nowrap text-right text-xs font-medium">
                      ...
                      >
                        <Transition name="slide-left" mode="out-in">
                          ...
                          >
                            <span
                              class="text-xs font-black text-gray-900 dark:text-white uppercase tracking-widest shrink-0"
                              >Delete template?</span
                            >
                            <div class="flex items-center gap-3">
                              <button
                                @click="confirmingId = null"
                                class="text-xs font-black uppercase text-gray-400 hover:text-gray-600 transition-colors"
                              >
                                Cancel
                              </button>
                              <button
                                @click="deleteTemplate(template.id, true)"
                                class="text-xs font-black uppercase bg-rose-600 hover:bg-rose-700 text-white px-3 py-1 rounded-lg transition-all shadow-sm shadow-rose-500/20"
                              >
                                Confirm
                              </button>
                            </div>
                          ...
                          <div v-else class="flex items-center justify-end gap-2">
                            <button
                              ...
                              :class="{
                                'text-primary-600 dark:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-900/20':
                                  !testingTemplates.has(template.id) && !testResults[template.id],
                                'text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/20':
                                  testResults[template.id]?.success,
                                'text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20':
                                  testResults[template.id] && !testResults[template.id].success,
                                'animate-pulse': testingTemplates.has(template.id)
                              }"
                              :title="$t('connections.testConnection')"
                            >
                              <RefreshCw
                                v-if="testingTemplates.has(template.id)"
                                class="w-4 h-4 animate-spin"
                              />
                              <CheckCircle2
                                v-else-if="testResults[template.id]?.success"
                                class="w-4 h-4"
                              />
                              <AlertCircle
                                v-else-if="
                            ...
                            <button
                              v-if="['mysql', 'postgres'].includes(template.type)"
                              @click="openReconfigure(template)"
                            ...
                            <button
                              @click="confirmingId = template.id"
          ...
          >
            ...
            >
              {{ $t('settings.global_connections.emptyTitle') }}
            </h3>
            <p class="text-xs text-gray-500 max-w-xs mb-6">
              {{ $t('settings.global_connections.emptyDesc') }}
            </p>
            <button
              @click="openForm()"
              class="px-6 py-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-xs font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest hover:border-indigo-500 hover:text-indigo-500 hover:shadow-lg hover:shadow-indigo-500/10 transition-all"
            >
    ...
    >
      ...
      <div
        class="px- py-1 flex items-center justify-between shrink-0 border-b border-gray-100 dark:border-gray-800"
      >
        <div class="flex items-center gap-4">
          <div
          ...
          <div>
            <h3 class="text-lg font-black text-gray-900 dark:text-white uppercase tracking-tight">
              {{
                editingTemplate
                  ? $t('settings.global_connections.edit')
                  : $t('settings.global_connections.create')
              }}
            </h3>
        ...
        <div class="flex items-end gap-4 flex-wrap">
          <div class="space-y-2 flex-1 min-w-[180px] max-w-sm">
            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1"
              >{{ $t('connections.databaseType') }} *</label
            >
            <div class="relative group">
              ...
              <ChevronDown
                class="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none group-hover:text-primary-500 transition-colors"
              />
            </div>
          </div>
          ...
          <div
            v-if="!editingTemplate && ['mysql', 'postgres'].includes(form.type || '')"
            class="shrink-0 pb-0.5"
          >
            <div
              class="flex items-center p-1 bg-gray-100 dark:bg-gray-800 rounded-xl overflow-x-auto"
            >
              <button
                @click="isUserConnectionMode = false"
              ...
              <button
                @click="isUserConnectionMode = true"
                class="px-5 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all gap-2 flex items-center justify-center whitespace-nowrap"
                :class="
                  isUserConnectionMode
      ...
      <div
        v-if="
          isUserConnectionMode &&
          (!editingTemplate || reconfigureMode) &&
          ['mysql', 'postgres'].includes(form.type || '')
        "
        class="flex-1 flex flex-col min-h-0 pt-2"
      >
        <SetupUserTemplate
        ...
        <div class="px-6 py-4 space-y-6 overflow-y-auto">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-2">
              <label
                class="block text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1"
                >{{ $t('connections.environment') }} (Optional)</label
              >
          ...
          <!-- Actions -->
          <div
            class="flex items-center justify-end gap-4 pt-4 border-t border-gray-100 dark:border-gray-800"
          >
            <button
              @click="closeForm"
...
const connectionGroups = computed(() => {
    ...
    filtered = filtered.filter(t => {
      ...
      const hasEnv =
        ...
        t.environment
          .split(',')
          .map(s => s.trim())
          .some(e => filterEnvs.value.includes(e))
  ...
  return [
    ...
    {
      id: 'dump',
      label: 'SQL Dump / Others',
      templates: filtered.filter(
        t => t.type === 'dump' || !['mysql', 'postgres', 'sqlite'].includes(t.type)
      )
    }
...
const isAllSelected = computed(() => {
  return (
    templates.value.length > 0 && templates.value.every(t => selectedTemplates.value.includes(t.id))
  )
})
...
watch(
  ...
  newTemplates => {
    if (
      ...
      Object.keys(testResults.value).length === 0 &&
      testingTemplates.value.size === 0
    ) {
