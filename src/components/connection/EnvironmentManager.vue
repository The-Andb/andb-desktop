  ...
  <div class="space-y-4">
    <div class="flex items-center justify-between">
      <h3 class="text-base font-semibold text-gray-900 dark:text-white">
        {{
          mode === 'global'
            ? $t('environments.global_configuration', 'Global Master List')
            : $t('environments.project_selection', 'Project Environments')
        }}
      </h3>
      <div v-if="mode === 'global'" class="flex items-center gap-2 relative">
        <div
          v-if="showSuggestions"
          class="absolute top-full right-0 mt-2 w-64 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-2xl z-[100] p-2 animate-in fade-in zoom-in-95 duration-200"
        >
          <div
            class="px-3 py-2 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-800 mb-1"
          >
            Suggestions
          </div>
          <div class="max-h-64 overflow-y-auto custom-scrollbar">
            ...
            >
              <div>
                ...
                <div
                  class="text-[10px] text-gray-500 dark:text-gray-400 truncate opacity-0 group-hover:opacity-100 transition-opacity"
                >
                  {{ s.description }}
                </div>
              </div>
          ...
          <div class="mt-1 pt-1 border-t border-gray-100 dark:border-gray-800">
            <button
              ...
              class="w-full text-left px-3 py-2 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 text-[10px] font-bold text-primary-600 uppercase tracking-widest flex items-center gap-2"
            >
        ...
        >
          ...
          <ChevronDown
            :class="['w-3 h-3 transition-transform', { 'rotate-180': showSuggestions }]"
          />
        </button>
      </div>
      <div v-else class="flex items-center gap-2">
        <button
          @click="emit('switchToGlobal')"
          class="text-[10px] font-bold text-primary-500 hover:underline uppercase tracking-widest flex items-center gap-1.5"
        >
          <Settings class="w-3.5 h-3.5" />
          {{ $t('environments.manage_global', 'Manage Global Env') }}
        </button>
      </div>
    </div>
    ...
    <div class="space-y-2">
      ...
      >
        <template #item="{ element: env }">
          <div
            class="group relative px-2 py-3 hover:bg-gray-100/30 dark:hover:bg-gray-800/30 transition-all duration-300 border-b border-gray-100 dark:border-gray-800/40 last:border-none"
          >
            <div class="flex items-center gap-4">
              <!-- Drag Handle & Toggle -->
              <div
                v-if="mode === 'global'"
                class="drag-handle cursor-move p-1 text-gray-300 hover:text-gray-500 dark:hover:text-gray-400 rounded transition-colors shrink-0"
              >
              ...
              <div class="flex-1 flex flex-col min-w-0">
                ...
                <template v-else>
                  <div
                    class="text-[13px] font-black text-gray-900 dark:text-white uppercase tracking-tight"
                  >
                    {{ env.name }}
                  </div>
                  <div class="text-[11px] text-gray-400 dark:text-gray-500 font-medium truncate">
                    {{ env.description || 'Global Master Environment' }}
                  </div>
                </template>
              </div>
              ...
              <div class="flex items-center gap-3 shrink-0">
                ...
                >
                  <Database
                    class="w-3.5 h-3.5 group-hover/btn:text-primary-500 transition-colors"
                  />
              ...
              >
                <button
                  ...
                  class="p-2 text-gray-400 hover:text-indigo-500 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-lg transition-all"
                >
                  <Copy class="w-4 h-4" />
                </button>
                <button
...
const getConnectionCount = (environmentName: string) => {
  const project = projectsStore.currentProject
  if (!project) return 0
  return appStore.resolvedConnections.filter(
    conn => conn.environment === environmentName && project.connectionIds.includes(conn.id)
  ).length
}
...
const updateEnvironment = (env: Environment) => {
  ...
  // Ensure unique names
  const duplicates = environments.value.filter(
    e => e.name.toUpperCase() === env.name.toUpperCase() && e.id !== env.id
  )
