<template>
  <div class="space-y-3">
    <div class="flex items-center justify-between">
      ...
      <button @click="addConnectionPair" class="btn btn-primary flex items-center">
        ...
        <div class="flex flex-col gap-4">
          ...
          <div class="flex items-center justify-between">
            ...
            >
              <button
                ...
                class="p-2 text-gray-400 hover:text-blue-500 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-all"
              ...
              >
                <Star class="w-4 h-4" :fill="pair.isDefault ? 'currentColor' : 'none'" />
              </button>
              <button
          ...
          <div class="flex items-center gap-3">
            <div class="flex-1 min-w-0 flex flex-col gap-1.5">
              <span class="text-[9px] font-bold text-gray-400 uppercase tracking-widest pl-1">{{
                $t('connectionPairs.source')
              }}</span>
              <select
              ...
              >
                ...
                <optgroup v-for="env in enabledEnvironments" :key="env.id" :label="env.name">
                  <option
                    v-for="conn in getConnectionsByEnv(env.name)"
                    :key="conn.id"
                    :value="conn.id"
                  >
                    {{ conn.name }}
            ...
            <div class="mt-4 shrink-0 px-1">
              ...
              <div class="relative w-6 h-6 flex items-center justify-center">
                <ArrowRight
                  class="w-4 h-4 text-gray-300 dark:text-gray-600 absolute"
                  :class="{ 'opacity-0': pair.status && pair.status !== 'idle' }"
                />
                <div
                  v-if="pair.status && pair.status !== 'idle'"
                  :class="getPairStatusClass(pair)"
                  class="w-2.5 h-2.5 rounded-full absolute shadow-[0_0_8px_rgba(0,0,0,0.1)]"
                ></div>
              </div>
            </div>
    ...
    <div
      v-if="defaultPair"
      class="card p-4 bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-800"
    >
...
const getConnectionsByEnv = (envName: string) => {
  ...
  return connections.value.filter(
    c => c.environment === envName && project.connectionIds.includes(c.id)
  )
}
...
const updatePair = (pair: ConnectionPair) => {
  ...
  // Ensure unique names
  const duplicates = connectionPairs.value.filter(p => p.name === pair.name && p.id !== pair.id)
