  ...
  <div class="space-y-6">
    ...
    <div class="space-y-2">
      <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1"
        >{{ $t('connections.environment') }} *</label
      >
      <div
        class="flex flex-wrap gap-2 pt-1 border border-gray-100 dark:border-gray-800/50 p-3 rounded-2xl bg-gray-50/30 dark:bg-gray-900/10"
      >
        <button
          ...
          type="button"
          @click="
            form.environment === env.name ? (form.environment = '') : (form.environment = env.name)
          "
      ...
      <p v-if="errors.environment" class="text-red-500 text-[10px] font-bold uppercase mt-1 ml-1">
        {{ errors.environment }}
      </p>
    </div>
    ...
    <div
      class="bg-indigo-50/50 dark:bg-indigo-900/10 rounded-2xl p-4 border border-indigo-100 dark:border-indigo-900/30"
    >
      <div class="flex items-center justify-between mb-3">
        <label
          class="block text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest flex items-center gap-2"
        >
          <LayoutTemplate class="w-4 h-4" />
          {{ $t('connections.template.source') }}
        </label>
        <router-link
          to="/settings?cat=templates"
          class="text-[9px] font-black text-indigo-500 hover:text-indigo-600 dark:hover:hover:text-indigo-400 uppercase tracking-wider flex items-center gap-1 transition-colors"
        >
    ...
    <div
      ...
      class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-gray-50/50 dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-800"
    >
      <div class="space-y-1">
      ...
      <div class="space-y-1">
        ...
        <div
          class="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 flex items-center gap-2"
        >
    ...
    <div class="space-y-6 pt-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-2">
          ...
          />
          <p v-if="errors.name" class="text-red-500 text-[10px] font-bold uppercase mt-1 ml-1">
            {{ errors.name }}
          </p>
        </div>
        ...
        <div class="space-y-2">
          <label
            class="block text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1"
            >{{ $t('connections.connectionTimeout') }}</label
          >
          <input
    ...
    <!-- Actions -->
    <div
      class="flex items-center justify-between pt-8 border-t border-gray-100 dark:border-gray-800 shrink-0"
    >
      <button
  ...
  return templatesStore.templates.filter(
    t =>
      ...
      t.environment
        .split(',')
        .map(s => s.trim())
        .includes(form.value.environment)
  )
})
...
watch(
  ...
  newId => {
    ...
    if (template) {
      ...
      form.value.ssh = (template.ssh as any) || {
        enabled: false,
        host: '',
        port: 22,
        username: '',
