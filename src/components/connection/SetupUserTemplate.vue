<template>
  <div class="flex flex-col h-full bg-white dark:bg-gray-900 transition-colors duration-300">
    <!-- Breadcrumb Header -->
    <div
      class="shrink-0 px-6 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between bg-white dark:bg-gray-950/50 backdrop-blur-xl sticky top-0 z-10"
    >
      <div
        class="flex items-center gap-3 text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest select-none"
      >
        <span
        ...
        <span
          class="hover:text-primary-600 dark:hover:text-primary-400 transition-colors cursor-pointer"
          >Global Connections</span
        >
        <ChevronRight class="w-3 h-3 text-gray-300 dark:text-gray-700" />
        <div
          class="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-400/10 px-3 py-1.5 rounded-full border border-emerald-100 dark:border-emerald-400/20 shadow-sm animate-in fade-in zoom-in duration-500"
        >
      ...
      <div class="relative group cursor-help">
        <div
          class="absolute -inset-1 bg-gradient-to-r from-emerald-500 to-teal-600 rounded-full blur opacity-20 group-hover:opacity-40 transition duration-1000 group-hover:duration-200"
        ></div>
        <div
          class="relative flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-gray-900 border border-emerald-200 dark:border-emerald-500/30 rounded-full shadow-sm"
        >
          ...
          <span
            class="text-[10px] font-black text-emerald-700 dark:text-emerald-400 uppercase tracking-[0.2em] bg-clip-text"
          >
            STRICT SAFETY
          </span>
        </div>
    ...
    <!-- Stepper Navigation -->
    <div
      class="shrink-0 px-6 py-3 flex items-center justify-center gap-3 bg-white dark:bg-gray-950 border-b border-gray-50 dark:border-gray-900"
    >
      <template v-for="step in totalSteps" :key="step">
        ...
        >
          <div
            ...
            :class="
              store.currentStep === step
                ? 'bg-blue-600 text-white ring-2 ring-blue-500/20 shadow-md shadow-blue-500/20'
                : store.currentStep > step
          ...
          <span
            class="text-[10px] font-black uppercase tracking-wider hidden md:block"
            :class="store.currentStep === step ? 'text-gray-900 dark:text-white' : 'text-gray-400'"
          >
        ...
        <div
          ...
          :class="store.currentStep > step ? 'bg-blue-600/30' : 'bg-gray-100 dark:bg-gray-800'"
        ></div>
      </template>
    </div>
    ...
    <div class="flex-1 overflow-y-auto px-8 py-4 custom-scrollbar bg-white dark:bg-gray-950">
      ...
      <div
        v-if="store.currentStep === SetupStep.INPUT"
      ...
      >
        ...
        <div class="grid grid-cols-2 gap-x-8 gap-y-6">
          <div class="col-span-2 space-y-2">
            <label
              class="block text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest ml-1"
              >{{ $t('connections.connectionName') }}</label
            >
            <input
              ...
              type="text"
              class="form-input"
              placeholder="e.g. Production Master"
            />
          </div>
          ...
          <div class="space-y-2">
            ...
            >
            <input
              v-model="store.adminCredentials.host"
              @focus="selectAll"
              type="text"
              class="form-input"
          ...
          <div class="space-y-2">
            ...
            >
            <input
              v-model.number="store.adminCredentials.port"
              @focus="selectAll"
              type="number"
              class="form-input"
              placeholder="3306"
          ...
          <div class="col-span-2 space-y-2">
            ...
            >
            <input
              v-model="store.adminCredentials.database"
              @focus="selectAll"
              type="text"
              class="form-input"
          ...
          <div
            v-if="!reconfigureMode"
            class="col-span-2 h-px bg-gray-100 dark:bg-gray-800/50 my-2"
          ></div>

          <div v-if="!reconfigureMode" class="col-span-2 space-y-2">
            <label
              class="block text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest ml-1"
              >{{ $t('restrictedUserSetup.restrictedUsername.label') }}</label
            >
            <div class="grid grid-cols-2 gap-4">
              <input
                v-model="store.restrictedUser.username"
                @focus="selectAll"
                type="text"
                class="form-input"
                placeholder="the_andb"
              />
              <div class="relative">
                <input
                ...
                />
                <button
                  @click="showRestrictedPass = !showRestrictedPass"
            ...
            <p class="text-[10px] text-gray-400 ml-1 italic leading-relaxed">
              {{ $t('restrictedUserSetup.description') }} (Generated automatically, you can change
              it)
            </p>
          </div>
          ...
          <div class="col-span-2 pt-4">
            ...
            >
              <div class="flex items-center gap-2">
                ...
                <span
                  ...
                  >SSH Tunnel (Optional)</span
                >
              </div>
              <label class="relative inline-flex items-center cursor-pointer group">
                <input
                  type="checkbox"
                  v-model="store.adminCredentials.ssh.enabled"
                  class="sr-only peer"
                />
                <div
                  class="w-8 h-4 bg-gray-200 dark:bg-gray-700 rounded-full peer-checked:bg-emerald-500 transition-colors"
                ></div>
                <div
            ...
            <div
              v-if="store.adminCredentials.ssh.enabled"
              class="grid grid-cols-2 gap-x-8 gap-y-6 animate-in slide-in-from-top-2 duration-300 mb-6"
            >
              <div class="space-y-2">
                <label
                ...
                >
                <input
                  v-model="store.adminCredentials.ssh.host"
                  @focus="selectAll"
                  type="text"
                  class="form-input"
              ...
              <div class="space-y-2">
                ...
                >
                <input
                  v-model.number="store.adminCredentials.ssh.port"
                  @focus="selectAll"
                  type="number"
                  class="form-input"
                  placeholder="22"
              ...
              <div class="space-y-2">
                ...
                >
                <input
                  v-model="store.adminCredentials.ssh.username"
                  @focus="selectAll"
                  type="text"
                  class="form-input"
                  placeholder="root"
              ...
              <div class="space-y-2">
                ...
                <div class="relative">
                  <input
                    v-model="store.adminCredentials.ssh.privateKeyPath"
                    @focus="selectAll"
                    type="text"
                    class="form-input pr-10"
              ...
              <p
                v-if="sshKeyWarning"
                class="col-span-2 text-[10px] font-bold text-amber-600 dark:text-amber-400 mt-1 flex items-start gap-1 p-2 bg-amber-50 dark:bg-amber-900/10 rounded-lg border border-amber-200 dark:border-amber-800 animate-in fade-in slide-in-from-top-1"
              >
      ...
      <div
        v-if="store.currentStep === SetupStep.CONFIG"
        class="space-y-8 animate-in fade-in slide-in-from-right-8 duration-500"
      >
        ...
        <div class="space-y-6">
          <div class="flex items-baseline justify-between">
            ...
            <span
              class="text-xs font-bold px-2 py-1 rounded bg-emerald-50 dark:bg-emerald-900/10 text-emerald-600 dark:text-emerald-400"
            >
              {{ securityLevelLabel }}
            </span>
          </div>
          ...
          <div class="space-y-3">
            <!-- Read Only (Protected) -->
            <div
              class="flex items-center justify-between p-5 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 opacity-80 cursor-not-allowed"
            >
              <div class="flex items-center gap-4">
                ...
                <div>
                  <div class="flex items-center gap-2">
                    ...
                    >
                    <span
                      class="text-xs font-black px-1.5 py-0.5 rounded bg-gray-200 dark:bg-gray-700 text-gray-500 uppercase"
                      >Mandatory</span
                    >
                  </div>
                  <p class="text-xs text-gray-500 mt-1">
                    Reads all schema definitions: Tables, Views, Procedures, Functions, Triggers,
                    and Events — without modifying data.
                  </p>
                </div>
              </div>
              <div
                class="relative inline-flex h-5 w-9 shrink-0 cursor-not-allowed rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none bg-emerald-500"
              >
                <span
                  class="pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out translate-x-4"
                ></span>
              </div>
            </div>
            ...
            <!-- Write/Alter -->
            <label
              class="flex items-center justify-between p-5 rounded-xl border-2 transition-all cursor-pointer group hover:bg-gray-50 dark:hover:bg-gray-800/50"
              :class="
                store.permissions.writeAlter
            ...
            >
              <div class="flex items-center gap-4">
                ...
                <div>
                  <span
                    class="text-sm font-bold text-gray-900 dark:text-white block group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors"
                    >Schema Change Support (ALTER TABLE)</span
                  >
                  <p class="text-xs text-gray-500 mt-1">
                    Allow TheAndb to suggest and apply table modifications for synchronization.
                  </p>
                </div>
              </div>
              <input type="checkbox" v-model="store.permissions.writeAlter" class="sr-only" />
              <div
                class="w-10 h-6 rounded-full transition-colors flex items-center p-1"
                :class="
                  store.permissions.writeAlter ? 'bg-indigo-500' : 'bg-gray-200 dark:bg-gray-700'
                "
              >
                <div
                  class="w-4 h-4 rounded-full bg-white shadow-sm transform transition-transform"
                  :class="store.permissions.writeAlter ? 'translate-x-4' : 'translate-x-0'"
                ></div>
              </div>
            </label>
            ...
            >
              <div class="flex items-center gap-4">
                ...
                <div>
                  <span
                    class="text-sm font-bold text-gray-900 dark:text-white block group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors"
                    >View Management</span
                  >
                  <p class="text-xs text-gray-500 mt-1">
                    Allow updating view definitions when differences are detected across
                    environments.
                  </p>
                </div>
              </div>
              ...
              <div
                class="w-10 h-6 rounded-full transition-colors flex items-center p-1"
                :class="
                  store.permissions.writeView ? 'bg-purple-500' : 'bg-gray-200 dark:bg-gray-700'
                "
              >
                <div
                  class="w-4 h-4 rounded-full bg-white shadow-sm transform transition-transform"
                  :class="store.permissions.writeView ? 'translate-x-4' : 'translate-x-0'"
                ></div>
              </div>
            </label>
            ...
            <label
              class="flex items-center justify-between p-5 rounded-xl border-2 transition-all cursor-pointer group hover:bg-gray-50 dark:hover:bg-gray-800/50"
              :class="
                store.permissions.writeRoutine
            ...
            >
              <div class="flex items-center gap-4">
                ...
                <div>
                  <span
                    class="text-sm font-bold text-gray-900 dark:text-white block group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors"
                    >Routine Management</span
                  >
                  <p class="text-xs text-gray-500 mt-1">
                    Allow updating Stored Procedures and Functions to ensure consistent business
                    logic.
                  </p>
                </div>
              </div>
              ...
              <div
                class="w-10 h-6 rounded-full transition-colors flex items-center p-1"
                :class="
                  store.permissions.writeRoutine ? 'bg-pink-500' : 'bg-gray-200 dark:bg-gray-700'
                "
              >
                <div
                  class="w-4 h-4 rounded-full bg-white shadow-sm transform transition-transform"
                  :class="store.permissions.writeRoutine ? 'translate-x-4' : 'translate-x-0'"
                ></div>
              </div>
            </label>
          </div>
        ...
        <div class="space-y-6">
          ...
          <div class="grid grid-cols-2 gap-6">
            ...
            >
              <div class="flex items-center justify-between w-full">
                ...
                <div
                  v-if="store.setupMode === 'auto'"
                  class="w-5 h-5 rounded-full bg-blue-500 shadow-sm ring-2 ring-white dark:ring-gray-900 flex items-center justify-center"
                >
                  <Check class="w-3 h-3 text-white" />
                </div>
              </div>
              <div>
                <h4 class="font-black text-base text-gray-900 dark:text-white mb-2">
                  Automatic Setup
                </h4>
                ...
                <div
                  class="mt-4 text-xs font-bold text-blue-600 dark:text-blue-400 bg-blue-100 dark:bg-blue-900 px-2 py-1 rounded inline-block"
                >
                  RECOMMENDED
                </div>
              </div>
            ...
            >
              <div class="flex items-center justify-between w-full">
                <div
                  class="p-3 rounded-xl bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300"
                >
                  <Terminal class="w-6 h-6" />
                </div>
                <div
                  v-if="store.setupMode === 'manual'"
                  class="w-5 h-5 rounded-full bg-gray-900 dark:bg-white shadow-sm ring-2 ring-white dark:ring-gray-900 flex items-center justify-center"
                >
          ...
          <div
            v-if="store.setupMode === 'auto'"
            class="p-5 bg-blue-50 dark:bg-gray-900 border border-blue-200 dark:border-blue-500/30 rounded-xl flex items-start gap-4 animate-in fade-in zoom-in duration-300"
          >
            ...
            <div class="space-y-2">
              <p class="text-xs text-blue-800 dark:text-blue-100 leading-relaxed font-medium">
                <strong class="text-blue-900 dark:text-blue-300 block mb-1"
                  >Volatile Memory Processing:</strong
                >
                Admin credentials provided are processed <strong>strictly in memory</strong> and are
                <strong>wiped immediately</strong> after setup.
              </p>
            </div>
          </div>
        </div>
        ...
        <div
          v-if="reconfigureMode || store.setupMode === 'auto'"
          class="space-y-6 animate-in fade-in slide-in-from-top-4 duration-500"
        >
          <h3 class="font-black text-gray-900 dark:text-white text-lg">3. Admin Authentication</h3>
          <div
            class="p-3 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/30 rounded-xl flex items-start gap-2.5"
          >
            ...
            <p class="text-[11px] text-amber-700 dark:text-amber-300 leading-relaxed font-medium">
              <template v-if="reconfigureMode">
                {{
                  $t('restrictedUserSetup.reconfigure.adminNote', {
                    username: store.restrictedUser.username
                  })
                }}
              </template>
              <template v-else>
                Admin credentials are required to create the restricted user. They are used only for
                this step and are <strong>not saved</strong>.
              </template>
            </p>
          </div>
          <div class="grid grid-cols-2 gap-x-8 gap-y-6">
            <div class="space-y-2">
              <label
                class="block text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest ml-1"
                >{{ $t('connections.username') }} (Admin)</label
              >
              <input
                v-model="store.adminCredentials.username"
                @focus="selectAll"
                type="text"
                class="form-input"
                placeholder="root"
            ...
            <div class="space-y-2">
              <label
                class="block text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest ml-1"
                >{{ $t('connections.password') }} (Admin)</label
              >
              <div class="relative">
                <input
                  v-model="store.adminCredentials.password"
                  @focus="selectAll"
      ...
      <div
        v-if="store.currentStep === SetupStep.EXECUTION"
        class="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500"
      >
        <!-- SQL Review Section -->
        <div
          class="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-6 border border-gray-100 dark:border-gray-800"
        >
          <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex items-center gap-4">
              <button
                @click="showPasswordsInScript = !showPasswordsInScript"
                class="text-xs font-bold flex items-center gap-1 transition-colors"
                :class="showPasswordsInScript ? 'text-amber-600' : 'text-gray-400'"
              >
          ...
          <div class="relative group">
            <div
              class="absolute right-3 top-3 p-1.5 rounded-lg bg-white/5 hover:bg-white/10 cursor-pointer transition-colors border border-white/5"
              @click="copyScript"
            ...
            <pre
              class="bg-gray-900 text-gray-300 p-5 rounded-2xl text-xs leading-relaxed font-mono overflow-x-auto border border-gray-800 shadow-inner custom-scrollbar max-h-48"
              >{{ maskedScript }}</pre
            >
          </div>
          ...
          <div v-if="store.setupMode === 'auto' && !executionDone" class="mt-6">
            ...
            <p class="text-[10px] text-gray-400 text-center mt-3">
              By clicking above, you authorize TheAndb to execute these commands using the provided
              admin credentials.
            </p>
          </div>
          ...
          <div
            v-if="store.setupMode === 'manual'"
            class="mt-4 p-4 bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-xl flex items-start gap-3"
          >
            ...
            <p class="text-[11px] text-blue-700 dark:text-blue-300 leading-relaxed font-medium">
              Please execute the script above on your database server manually. Once finished, run
              the verification suite below to confirm connection.
            </p>
          </div>
        </div>
        ...
        <div
          v-if="isExecuting || executionDone || store.setupMode === 'manual'"
          class="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in slide-in-from-bottom-4 duration-500"
        >
          <!-- Logs Terminal (Only for Auto) -->
          <div v-if="store.setupMode === 'auto'" class="space-y-3">
            <h5 class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">
              Execution Status
            </h5>
            <div
              class="bg-black/90 rounded-2xl p-4 font-mono text-[10px] h-64 overflow-y-auto border border-gray-800 shadow-inner custom-scrollbar"
            >
              <div
                v-for="(log, i) in store.executionLogs"
                :key="i"
                class="text-gray-300 mb-1 break-all"
              >
                <span class="text-gray-600 mr-2">[{{ new Date().toLocaleTimeString() }}]</span>
                <span :class="log.includes('Error') ? 'text-red-400' : 'text-gray-300'"
                  >> {{ log }}</span
                >
              </div>
          ...
          <div class="space-y-3" :class="store.setupMode === 'manual' ? 'md:col-span-2' : ''">
            ...
            <div
              class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl overflow-hidden shadow-sm divide-y divide-gray-100 dark:divide-gray-800"
            >
              <!-- Check 1: Connection -->
              <div class="p-4 flex items-center justify-between">
                ...
                <div>
                  <span
                    v-if="store.verificationResults.baseConn === 'pass'"
                    class="text-[10px] font-black text-green-500 uppercase"
                    >Passed</span
                  >
                  <span
                    v-else-if="store.verificationResults.baseConn === 'fail'"
                    class="text-[10px] font-black text-red-500 uppercase"
                    >Failed</span
                  >
              ...
              <div class="p-4 flex items-center justify-between">
                ...
                <div>
                  <span
                    v-if="store.verificationResults.schemaRead === 'pass'"
                    class="text-[10px] font-black text-green-500 uppercase"
                    >Passed</span
                  >
                  <span
                    v-else-if="store.verificationResults.schemaRead === 'fail'"
                    class="text-[10px] font-black text-red-500 uppercase"
                    >Failed</span
                  >
                  <span v-else class="text-[10px] font-black text-gray-300 uppercase">Pending</span>
              ...
              <div class="p-4 flex items-center justify-between">
                ...
                <div>
                  <span
                    v-if="store.verificationResults.sandboxTest === 'pass'"
                    class="text-[10px] font-black text-green-500 uppercase"
                    >Active</span
                  >
                  <span
                    v-else-if="store.verificationResults.sandboxTest === 'fail'"
                    class="text-[10px] font-black text-red-500 uppercase"
                    >Bypassed</span
                  >
                  <span v-else class="text-[10px] font-black text-gray-300 uppercase">Pending</span>
            ...
            <div
              v-if="verificationDone && !allChecksPassed"
              class="p-3 bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 rounded-xl"
            >
              <p class="text-[10px] text-red-600 dark:text-red-400 font-bold leading-tight">
                Verification failed. Please check if the script was executed correctly and the user
    ...
    <!-- Actions Footer -->
    <div
      class="px-8 py-6 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between shrink-0 bg-white dark:bg-gray-950"
    >
      <button
        @click="prev"
      ...
      <div class="flex items-center gap-6">
        ...
        <button
          ...
          :class="[
            currentActionIsTest
              ? adminTestResult === 'pass'
                ? 'bg-green-600 text-white shadow-lg shadow-green-500/20'
                : adminTestResult === 'fail'
...
watch(
  [() => store.adminCredentials, connectionName],
  () => {
...
onMounted(() => {
  ...
  if (props.initialData) {
    if (props.initialData.name) {
      connectionName.value = props.reconfigureMode
        ? props.initialData.name
        : `${props.initialData.name} (Secure)`
    }
...
const securityLevelLabel = computed(() => {
  if (store.permissions.writeAlter && store.permissions.writeView && store.permissions.writeRoutine)
    return 'Advanced'
...
const allChecksPassed = computed(() => {
  return (
    store.verificationResults.baseConn === 'pass' &&
...
const startExecution = async () => {
  ...
  try {
    ...
    addLog(
      $t('restrictedUserSetup.logs.connecting', {
        host: store.adminCredentials.host,
        user: store.adminCredentials.username
      })
    )
    ...
    if (!testAdmin || !testAdmin.success)
      throw new Error('Admin connection failed. Please check credentials.')
