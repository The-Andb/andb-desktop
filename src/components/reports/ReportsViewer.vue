<template>
  <div
    v-if="isOpen"
    class="fixed inset-0 z-[100] bg-gray-50 dark:bg-gray-900 flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-300"
  >
    <!-- Header -->
    <div
      class="h-14 border-b border-gray-200 dark:border-gray-800 flex items-center px-4 justify-between bg-white dark:bg-[#0F172A]"
    >
    ...
    <div class="flex-1 flex overflow-hidden">
      <!-- Sidebar: Report List -->
      <div
        class="w-1/4 min-w-[280px] max-w-[350px] border-r border-gray-200 dark:border-gray-800 flex flex-col bg-white dark:bg-gray-900"
      >
        <div
        ...
        <div
          v-if="reports.length === 0"
          class="flex-1 flex flex-col items-center justify-center p-6 text-center opacity-60"
        >
              ...
              <div
                ...
                :class="
                  selectedReport?.name === report.name
                    ...
                    : 'bg-gray-100 dark:bg-gray-800 text-gray-500 group-hover:bg-white dark:group-hover:bg-gray-700 shadow-sm'
                "
              >
                JSON
              </div>
      ...
      <div
        ...
        class="flex-1 flex items-center justify-center bg-gray-50 dark:bg-[#0B1121]"
      >
      ...
      <div v-else-if="reportData" class="flex-1 overflow-y-auto bg-gray-50 dark:bg-[#0B1121] p-8">
        ...
        <div class="max-w-7xl mx-auto space-y-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <!-- Status -->
            <div
              class="bg-white dark:bg-slate-800/50 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm backdrop-blur-sm"
            >
              <h4 class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">
                {{ $t('reports.syncStatus') }}
              </h4>
              <div
                class="text-3xl font-black tracking-tight"
                :class="isSync ? 'text-emerald-500' : 'text-amber-500'"
              >
            ...
            <!-- New -->
            <div
              class="bg-white dark:bg-slate-800/50 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm backdrop-blur-sm group hover:border-emerald-500/50 transition-colors"
            >
              <h4 class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">
                {{ $t('reports.createdObjects') }}
              </h4>
            ...
            <!-- Updated -->
            <div
              class="bg-white dark:bg-slate-800/50 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm backdrop-blur-sm group hover:border-blue-500/50 transition-colors"
            >
              <h4 class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">
                {{ $t('reports.modifiedUnits') }}
              </h4>
              <div class="text-3xl font-black tracking-tight text-blue-500">
                {{ stats.updated }}
              </div>
            </div>
            ...
            <!-- Deprecated -->
            <div
              class="bg-white dark:bg-slate-800/50 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm backdrop-blur-sm group hover:border-red-500/50 transition-colors"
            >
              <h4 class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">
                {{ $t('reports.terminated') }}
              </h4>
              <div class="text-3xl font-black tracking-tight text-red-500">
                {{ stats.deprecated }}
              </div>
            </div>
          </div>
          ...
          <!-- Chart -->
          <div
            class="bg-white dark:bg-slate-800/50 rounded-3xl p-8 border border-slate-200 dark:border-slate-700 shadow-lg"
          >
          ...
          <div class="grid grid-cols-1 xl:grid-cols-2 gap-8">
            <div
              ...
              :key="key"
              class="bg-white dark:bg-slate-800/50 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden flex flex-col"
            >
              <div
                class="p-6 border-b border-slate-100 dark:border-slate-700/50 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/30"
              >
                <h3 class="text-lg font-bold flex items-center gap-2 uppercase">{{ key }}</h3>
                <span
                  class="px-3 py-1 bg-slate-200 dark:bg-slate-700 rounded-full text-[10px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300"
                >
              ...
              <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-6 flex-1">
                ...
                <div class="bg-emerald-500/5 rounded-xl p-4 border border-emerald-500/10">
                  <h4
                    class="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase mb-4 flex items-center gap-2"
                  >
                    <span class="w-2 h-2 rounded-full bg-emerald-500"></span>
                    {{ $t('common.status.newSource') }}
                  </h4>
                  <ul v-if="section.NEW && section.NEW.length" class="space-y-1">
                    <li
                      v-for="item in section.NEW"
                      :key="item"
                      class="text-xs font-mono py-1.5 px-2 bg-white/50 dark:bg-black/20 rounded border border-emerald-500/10 dark:border-emerald-500/20 truncate"
                    >
                      {{ item }}
                    </li>
                  </ul>
                  <div v-else class="text-[10px] italic text-slate-400 p-2">
                    {{ $t('reports.noChanges') }}
                  </div>
                </div>
                ...
                <div class="bg-blue-500/5 rounded-xl p-4 border border-blue-500/10">
                  <h4
                    class="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase mb-4 flex items-center gap-2"
                  >
                    <span class="w-2 h-2 rounded-full bg-blue-500"></span>
                    {{ $t('common.status.modified') }}
                  </h4>
                  <ul v-if="section.UPDATED && section.UPDATED.length" class="space-y-1">
                    <li
                      v-for="item in section.UPDATED"
                      :key="item"
                      class="text-xs font-mono py-1.5 px-2 bg-white/50 dark:bg-black/20 rounded border border-blue-500/10 dark:border-blue-500/20 truncate"
                    >
                      {{ item }}
                    </li>
                  </ul>
                ...
                <div class="bg-red-500/5 rounded-xl p-4 border border-red-500/10">
                  <h4
                    class="text-xs font-bold text-red-600 dark:text-red-400 uppercase mb-4 flex items-center gap-2"
                  >
                    <span class="w-2 h-2 rounded-full bg-red-500"></span>
                    {{ $t('common.status.deprecatedTarget') }}
                  </h4>
                  <ul v-if="section.DEPRECATED && section.DEPRECATED.length" class="space-y-1">
                    <li
                      v-for="item in section.DEPRECATED"
                      :key="item"
                      class="text-xs font-mono py-1.5 px-2 bg-white/50 dark:bg-black/20 rounded border border-red-500/10 dark:border-red-500/20 truncate"
                    >
                      {{ item }}
                    </li>
                  </ul>
