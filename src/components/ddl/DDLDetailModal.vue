<template>
  ...
  >
    <div
      class="bg-white dark:bg-gray-800 rounded-lg w-full max-w-5xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col"
    >
      <!-- Header -->
      <div
        class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shrink-0"
      >
          ...
          <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">
              {{
                $t('ddlDetail.details', {
                  type: item?.type
                    ? $t(`navigation.ddl.${item.type.toLowerCase()}`)
                    : $t('navigation.ddl.tables')
                })
              }}
            </p>
          </div>
      ...
      <div class="overflow-y-auto flex-1 min-h-0">
        <!-- Status Info Bar -->
        <div
          class="px-6 py-3 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50 shrink-0"
        >
        ...
        <div v-if="activeTab === 'code'" class="p-6 space-y-6">
          <div v-if="ddlStatements.length > 0">
            <div class="flex items-center justify-between mb-2">
              ...
              <div class="flex gap-2">
                <button
                  @click="copyAllDDL"
                  class="text-xs flex items-center gap-1 text-primary-600 hover:text-primary-700"
                >
            ...
            <div class="space-y-4">
              <div v-for="(stmt, index) in ddlStatements" :key="index" class="relative group">
                ...
                >
                  <button
                    @click="copyDDL(stmt)"
                    class="p-1.5 bg-gray-800 text-white rounded hover:bg-gray-700"
                    title="Copy"
                  >
                    <Copy class="w-3 h-3" />
                  </button>
          ...
          <!-- Schema Diff -->
          <div
            v-if="item?.status === 'different' && schemaDiff"
            class="border-t border-gray-200 dark:border-gray-700 pt-6"
          >
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
              {{ $t('ddlDetail.schemaDiff') }}
            </h3>
            <div class="grid grid-cols-2 gap-4">
              <div>
                <div class="flex items-center gap-2 mb-2">
                  <div class="w-2 h-2 rounded-full bg-blue-500"></div>
                  <h4 class="text-xs font-bold uppercase text-gray-500">
                    {{ $t('common.source') }}
                  </h4>
                </div>
                <div
                  class="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 border border-gray-200 dark:border-gray-700 max-h-64 overflow-y-auto"
                >
                  <pre class="text-xs font-mono text-gray-600 dark:text-gray-300">{{
                    formatSchema(schemaDiff.source)
                  }}</pre>
                </div>
              </div>
              <div>
                ...
                >
                  <pre class="text-xs font-mono text-gray-600 dark:text-gray-300">{{
                    formatSchema(schemaDiff.target)
                  }}</pre>
                </div>
              </div>
            </div>
        ...
        <div
          ...
          class="h-full flex flex-col min-h-0 bg-gray-50/50 dark:bg-gray-900/50"
        >
          <DDLVisualizer :table-name="item?.name || ''" :columns="parsedColumns" />
        </div>
      </div>

      <!-- Footer -->
      <div
        class="flex items-center justify-end gap-3 p-4 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shrink-0"
      >
        <button
          @click="close"
        ...
        <button
          v-if="ddlStatements.length > 0"
          @click="applyChanges"
          class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-lg text-sm font-medium shadow-sm transition-colors flex items-center gap-2"
        >
...
watch(
  ...
  async statements => {
    const ddl =
      statements.find(s => s.toUpperCase().includes('CREATE TABLE')) || statements[0] || ''
