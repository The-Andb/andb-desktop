<template>
  <div class="h-full w-full bg-white dark:bg-gray-900 flex flex-col overflow-hidden">
    <!-- Visual Header -->
    <div
      class="px-4 py-2 bg-gray-100 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between shrink-0"
    >
      <div class="flex items-center gap-3">
        ...
        <div>
          <h3 class="font-bold text-sm text-gray-900 dark:text-white tracking-tight">
            {{ tableName }}
          </h3>
        </div>
      </div>
    ...
    <div v-if="columns.length > 0" class="flex-1 overflow-auto">
      <table class="w-full text-left border-collapse table-fixed">
        <thead
          class="bg-gray-50 dark:bg-gray-800 sticky top-0 z-10 font-bold text-xs uppercase text-gray-500 dark:text-gray-400 tracking-wider"
        >
          <tr>
            <th
              v-for="(header, i) in [
        ...
        <tbody class="text-xs font-mono divide-y divide-gray-100 dark:divide-gray-800">
          <tr
            ...
            :key="col.name"
            class="hover:bg-blue-50 dark:hover:bg-blue-900/20 group transition-colors"
          >
            <td class="px-4 py-2 text-gray-400 text-center">{{ index + 1 }}</td>
            <td
              class="px-4 py-2 font-semibold text-gray-700 dark:text-gray-200 group-hover:text-primary-600 truncate"
            >
            ...
            <td class="px-4 py-2 text-blue-600 dark:text-blue-400 font-bold uppercase truncate">
              {{ col.type }}
            </td>
            ...
            <td class="px-2 py-2 text-center">
              <div
                class="w-3 h-3 border border-gray-300 dark:border-gray-600 rounded mx-auto"
                :class="{ 'bg-yellow-500 border-yellow-500': col.pk }"
              ></div>
            </td>
            <td class="px-2 py-2 text-center">
              <div
                class="w-3 h-3 border border-gray-300 dark:border-gray-600 rounded mx-auto"
                :class="{ 'bg-blue-500 border-blue-500': col.notNull }"
              ></div>
            </td>
            <td class="px-2 py-2 text-center">
              <div
                class="w-3 h-3 border border-gray-300 dark:border-gray-600 rounded mx-auto"
                :class="{ 'bg-blue-500 border-blue-500': col.unique }"
              ></div>
            </td>
            <td class="px-2 py-2 text-center">
              <div
                class="w-3 h-3 border border-gray-300 dark:border-gray-600 rounded mx-auto"
                :class="{ 'bg-blue-500 border-blue-500': col.unsigned }"
              ></div>
            </td>
            <td class="px-2 py-2 text-center">
              <div
                class="w-3 h-3 border border-gray-300 dark:border-gray-600 rounded mx-auto"
                :class="{ 'bg-blue-500 border-blue-500': col.autoIncrement }"
              ></div>
            </td>
    ...
    <div
      v-else
      class="text-center text-gray-400 p-8 flex flex-col items-center justify-center h-full"
    >
...
const tableResizer = useTableResizer(
  'ddl-visualizer-table',
  [50, 180, 150, 40, 40, 40, 40, 40, 150, 250]
)
</script>
<style scoped>
.resize-handle {
